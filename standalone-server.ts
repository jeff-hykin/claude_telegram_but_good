...
function execSync(cmd: string, opts?: { cwd?: string; timeout?: number }): string {
...
// === Session registry ===
const sessions = new Map<string, { conn: Deno.UnixConn; info: SessionInfo }>()
...
const BOOT_ACCESS: Access | null = STATIC
  ? (() => {
      ...
      if (a.dmPolicy === "pairing") {
        Deno.stderr.writeSync(new TextEncoder().encode("telegram server: static mode — dmPolicy \"pairing\" downgraded to \"allowlist\"\n"))
...
const toolExecutor = createToolExecutor(
  ...
  async (text, chat_id) => {
    ...
    if (onReply) {
      try { await onReply({ text, chat_id } as any, bot, getCommandState()) } catch { /* ignore */ }
    }
  },
)
...
function getCommandState(): Record<string, unknown> {
  return {
    ...
    SESSION_PID: Deno.pid, SESSION_CWD: Deno.cwd(),
    ...
    execSync, randomHex, homedir: () => HOME, PLUGIN_VERSION,
  }
}
...
function deliverPermissionToFocused(params: { request_id: string; tool_name: string; description: string; input_preview: string }): boolean {
  ...
  const session = sessions.get(focusedSessionId)
  if (!session) return false
  sendIpc(session.conn, { type: "permission_request", ...params })
  return true
}
...
function handleShimMessage(conn: Deno.UnixConn, msg: IpcMessage): void {
  switch (msg.type) {
    case "register": {
      const info = msg.session
      dbg("IPC", "session registered:", info.id, "pid:", info.pid, "cwd:", info.cwd)
      sessions.set(info.id, { conn, info })
      Deno.stderr.writeSync(new TextEncoder().encode(`telegram server: session ${info.id} connected (PID ${info.pid}, ${info.cwd})\n`))
    ...
    case "permission_request": {
      ...
      for (const chat_id of access.allowFrom) {
        void bot.api.sendMessage(chat_id, text, { reply_markup: keyboard }).catch((e: unknown) => {
          Deno.stderr.writeSync(new TextEncoder().encode(`permission_request send to ${chat_id} failed: ${e}\n`))
        })
      }
      break
    }
...
function handleEnableTelegramByDefault(enabled: boolean): ToolResult {
  ...
  if (enabled) {
    ...
    try {
      ...
      const result = new Deno.Command("sh", {
        args: ["-c", "which claude"],
        env: { ...Object.fromEntries(Deno.env.toObject ? Object.entries(Deno.env.toObject()) : []), PATH: cleanPath, HOME },
        stdout: "piped",
        stderr: "piped",
      }).outputSync()
      realClaude = new TextDecoder().decode(result.stdout).trim()
      if (!realClaude) throw new Error("not found")
    } catch {
...
bot.on("callback_query:data", async ctx => {
  ...
  if (errMatch) {
    ...
    let isCustom = false
    try { Deno.statSync(join(HOME, ".claude", "telegram", "custom_commands", errInfo.cmdName + ".js")); isCustom = true } catch { /* not custom */ }
  ...
  const target = sessions.get(focusedSessionId ?? "")
  if (target) {
    sendIpc(target.conn, { type: "permission_reply", request_id: request_id!, behavior: behavior! })
  }
...
bot.on("message:text", async ctx => {
  ...
  if (onMsg) {
    try { await onMsg(ctx as any, bot, getCommandState()) } catch { /* ignore */ }
  }
...
bot.catch(err => {
  ...
  Deno.stderr.writeSync(new TextEncoder().encode(`telegram server: handler error (polling continues): ${err.error}\n`))
})

// === IPC Accept Loop ===

async function handleConnection(conn: Deno.UnixConn): Promise<void> {
...
// Approval polling
if (!STATIC) setInterval(() => checkApprovals(bot), 5000)
...
;(async () => {
  for await (const conn of listener) {
    handleConnection(conn as Deno.UnixConn)
  }
})()
    ...
    try {
      ...
      await bot.start({
        onStart: (info: { username: string }) => {
          ...
          Deno.stderr.writeSync(new TextEncoder().encode(`telegram server: polling as @${info.username} (standalone)\n`))
          ...
          ).catch((e: unknown) => dbg("COMMANDS", "setMyCommands failed:", e))
        },
      })
      return
    } catch (err) {
      if (err instanceof GrammyError && err.error_code === 409) {
        ...
        Deno.stderr.writeSync(new TextEncoder().encode(`telegram server: 409 Conflict, retrying in ${delay / 1000}s\n`))
