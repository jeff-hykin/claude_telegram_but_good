...
import { readConfig, getConfig, setConfig } from "./lib/config.ts"
import { stringify as stringifyYaml } from "@std/yaml"
import { startService, stopService, restartService, serviceStatus } from "./lib/daemon.ts"
import { createSession, attachSession, listDtachSockets } from "./lib/dtach.ts"
import { onboard, isOnboarded } from "./lib/onboard.ts"
import { PID_FILE, IPC_SOCK } from "./lib/protocol.ts"
  ...
  case "resume": {
    if (args[0]) {
      ...
      try {
        ...
        attachSession(selected as string)
      } catch {
