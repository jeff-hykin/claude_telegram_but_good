...
export const commands = {
    approve_user: (event, core) => {
        ...
        if (!match) {
            return reply(event.chatId, "Usage: /approve_user one_time_password:<token>")
        }

        const submitted = match[1]
        const otpMatch = submitted.match(/^one_time_password:(.+)$/)
        if (!otpMatch) {
            return reply(event.chatId, "Invalid format. Expected: one_time_password:<token>")
        }
        const token = otpMatch[1]

        const pending = core.chatState?.pendingOtps?.[token]
        if (!pending) {
            return reply(event.chatId, "No approval is pending for that token. Run `cbg onboard` or `cbg authorize` first.")
        }
