...
export const commands = {
    set_command_center: async (event, core) => {
        ...
        if (chatType !== "supergroup") {
            return { effects: [sendEffect(event.replyTo, "This command must be sent in a supergroup with Topics enabled.", { parse_mode: "HTML" })] }
        }
        ...
        try {
            ...
            return { effects: [sendEffect(event.replyTo, "Could not verify group settings. Make sure I have admin rights.", { parse_mode: "HTML" })] }
        }

        // Verify forum mode is enabled
        if (!chatInfo.is_forum) {
            return { effects: [sendEffect(event.replyTo, "This group needs Topics enabled. Go to group settings → Topics → toggle on, then try again.", { parse_mode: "HTML" })] }
        }
        ...
        try {
            ...
            return { effects: [sendEffect(event.replyTo, "Could not verify my admin status. Please promote me to admin and try again.", { parse_mode: "HTML" })] }
        }

        if (botMember.status !== "administrator" && botMember.status !== "creator") {
            return { effects: [sendEffect(event.replyTo, "I need admin rights to manage topics. Please promote me to admin and try again.", { parse_mode: "HTML" })] }
        }
