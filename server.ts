...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description: 'Create or remove a shell wrapper so that `claude` always starts with --channels plugin:telegram@claude-plugins-official. When enabled, creates a wrapper script that prepends the --channels flag and puts it on PATH via a shell startup file. When disabled, removes the wrapper. Returns the current state after the change.',
      inputSchema: {
        type: 'object',
        properties: {
...
mcp.setRequestHandler(CallToolRequestSchema, async req => {
  ...
  try {
    switch (req.params.name) {
      ...
      case 'enable_telegram_by_default': {
        ...
        if (enabled) {
          ...
          try {
            // Exclude our wrapper from the search
          ...
          return { content: [{ type: 'text', text: `Enabled. Wrapper at ${wrapperPath} → ${realClaude} ${CHANNELS_FLAG}\nNew shells will use this automatically. Run \`source ~/.zshrc\` or \`source ~/.bashrc\` to apply now.` }] }
        } else {
          // Remove wrapper
          ...
          return { content: [{ type: 'text', text: `Disabled. Wrapper removed. New shells will use the original claude binary.` }] }
        }
      }
      case 'new_command': {
