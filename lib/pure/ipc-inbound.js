    ...
    switch (msg.type) {
        ...
        case "hook_event": {
            ...
            return [{
                ...
                // Claude Code stamps a unique `tool_use_id` on each
                // tool invocation and passes it through to both the
                // PreToolUse and PostToolUse hook events. The spinner
                // policy uses it to replace a pre-tool item with its
                // post-tool counterpart in place (so we don't show
                // both "Reading foo.js" and "☑️ Read foo.js" stacked).
                toolUseId: data.tool_use_id ?? null,
                // Claude Code stamps every hook payload with the path to
                // that session's transcript JSONL. It's the only exact
                // source for how much context the session is carrying
                // (see lib/pure/context-usage.js), so it gets stashed on
                // the session at Stop.
        ...
        case "create_interval_hook": {
            return [{
                ...
                code: msg.code,
                rule: msg.rule,
                timeoutMs: msg.timeoutMs ?? null,
                _conn: conn,
            }]
        }

        case "deactivate_interval_hook": {
            return [{
