...
export function formatPreToolUse(event) {
    ...
    // their primary argument (<code>value</code>) so the spinner
    // item stays a single line.
    ...
    if (tool === "TaskGet" || tool === "TaskStop" || tool === "TaskOutput") {
        ...
        const suffix = taskId ? ` <code>${esc(String(taskId))}</code>` : ""
        return `<i>\uD83D\uDCCB ${esc(tool)}</i>${suffix}`
    }
...
export function formatPostToolUse(event) {
    ...
    if (tool === "TaskGet" || tool === "TaskStop" || tool === "TaskOutput") {
        const taskId = input?.task_id ?? input?.taskId ?? null
        const suffix = taskId ? ` <code>${esc(String(taskId))}</code>` : ""
        return `<i>${status} ${esc(tool)}</i>${suffix}`
    }
