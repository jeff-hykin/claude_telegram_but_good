...
export function translateTelegramMessage(ctx) {
    ...
    if (msg) {
        ...
        if (msg.forum_topic_created) {
            return [{
                ...
                chatType: chat.type,
                threadId: msg.message_thread_id ?? null,
                topicTitle: msg.forum_topic_created.name ?? "",
                _ctx: ctx,
            }]
        }
        ...
        if (msg.forum_topic_edited) {
            return [{
                ...
                chatType: chat.type,
                threadId: msg.message_thread_id ?? null,
                newTitle: msg.forum_topic_edited.name ?? null,
                _ctx: ctx,
            }]
        }
    }
    ...
    // The bot's own membership in a chat changed. Only the transition
    // into a chat matters downstream; the handler decides what counts as
    // "joined", so both statuses are passed through untouched.
    ...
    if (membership) {
        ...
        return [{
            ...
            chatId: String(chat.id),
            chatType: chat.type,
            chatTitle: safeName(chat.title) ?? null,
            userId: String(membership.from?.id ?? ""),
            oldStatus: membership.old_chat_member?.status ?? null,
            newStatus: membership.new_chat_member?.status ?? null,
            _ctx: ctx,
        }]
    }
