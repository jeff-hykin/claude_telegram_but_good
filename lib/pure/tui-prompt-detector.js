...
//   - file_edit: "Do you want to make this edit to <file>?" (Yes/No/Yes+settings)
//   - proceed: "Do you want to proceed?" (Yes/No)
...
export function detectPrompts(text) {
    ...
    // Claude Code shows this when requesting permission to edit a file
    // it doesn't have blanket permission for. Same numbered menu as
    // file_create. Auto-answer "1" (Yes).
    const editMatch = text.match(/Do you want to make this edit to ([^\n?]+)\?/i)
    if (editMatch) {
        prompts.push({
            type: "file_edit",
            file: editMatch[1].trim(),
            autoAnswer: "1",
        })
    }

    // "Do you want to proceed?"
    // Generic confirmation prompt. Auto-answer "1" (Yes).
