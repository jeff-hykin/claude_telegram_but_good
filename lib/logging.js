...
const { scrubText } = await versionedImport("./scrub.js", import.meta)
...
export function dbg(label, ...args) {
    ...
    const line = `[TG-DBG ${ts}] ${label}: ${args.map(formatArg).join(" ")}\n`
    Deno.stderr.writeSync(encoder.encode(line))
    const scrubbedLine = scrubText(line)
    try {
        Deno.writeTextFileSync(paths.LOG_FILE, scrubbedLine, { append: true })
    } catch (e) {
        Deno.stderr.writeSync(encoder.encode(`[TG-DBG ${ts}] LOG: write to ${paths.LOG_FILE} failed: ${e}\n`))
    }
}

// Error instances have non-enumerable `message`/`stack`, so JSON.stringify
// silently drops them and returns "{}". Surface them explicitly, and keep
// any enumerable custom props (GrammyError stores method/payload/etc. on
// `this` and they serialize fine) so existing rich errors still show up.
function formatArg(a) {
    if (a instanceof Error) {
        const skip = new Set(["stack", "message", "name"])
        const extras = JSON.stringify(a, Object.getOwnPropertyNames(a).filter(k => !skip.has(k)))
