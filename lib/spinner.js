...
async function sendSpinnerMessage(core, chatId, sessionId, threadId) {
    ...
    const opts = { format: "markdown", silent: true }
    if (threadId != null) { opts.threadId = threadId }
    let sent
    try {
        sent = await core.bot.sendText(chatId, guardedHeader, opts)
    } catch (e) {
        const desc = String(e?.description ?? e?.message ?? "")
        const isParseError = e?.error_code === 400 && /can't parse entities/i.test(desc)
        if (isParseError) {
            dbg("SPINNER", "sendText parse error, retrying as plain text:", desc)
            const plainOpts = { silent: true }
            if (threadId != null) { plainOpts.threadId = threadId }
            const plainText = headerHtml
                .replace(/\\_/g, "_").replace(/\\\*/g, "*").replace(/\\`/g, "`").replace(/\\\[/g, "[").replace(/\\\\/g, "\\")
                .replace(/\*/g, "").replace(/_/g, "").replace(/`/g, "")
            try {
                sent = await core.bot.sendText(chatId, plainText, plainOpts)
            } catch (e2) {
                dbg("SPINNER", "sendText plain-text retry also failed:", e2)
                return
            }
        } else {
...
async function appendSpinnerItem(core, sessionId, rendered, ts, toolUseId) {
    ...
    try {
        ...
        const isParseError = e?.error_code === 400 && /can't parse entities/i.test(desc)
        if (isParseError) {
            dbg("SPINNER", "editText parse error, retrying as plain text")
            const plainText = text
                .replace(/\\_/g, "_").replace(/\\\*/g, "*").replace(/\\`/g, "`").replace(/\\\[/g, "[").replace(/\\\\/g, "\\")
                .replace(/\*/g, "").replace(/_/g, "").replace(/`/g, "")
            try {
                await core.bot.editText(nextSpinner.chatId, nextSpinner.messageId, plainText, {})
            } catch (e2) {
                dbg("SPINNER", "editText plain-text retry also failed:", e2)
            }
        } else {
