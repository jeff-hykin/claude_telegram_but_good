...
const { renderMarkdownToEntities } = await versionedImport("./pure/telegram-markdown.js", import.meta)
...
const { maybePrependGeneralHeader } = await versionedImport("./effects/telegram-outbound.js", import.meta)
...
const SPINNER_VERBS = [
    ...
    "Thinking", "Tinkering", "Tomfoolering", "Transmuting", "Undulating",
    "Unfurling", "Vibing", "Waddling", "Wandering", "Whirring",
    "Whisking", "Wibbling", "Wrangling", "Zesting", "Zigzagging",
]

let shuffledVerbIndices = []
let verbShufflePos = 0

function refillVerbShuffle() {
    shuffledVerbIndices = SPINNER_VERBS.map((_, i) => i)
    for (let i = shuffledVerbIndices.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1))
        ;[shuffledVerbIndices[i], shuffledVerbIndices[j]] = [shuffledVerbIndices[j], shuffledVerbIndices[i]]
    }
    verbShufflePos = 0
}

function randomVerb() {
    if (verbShufflePos >= shuffledVerbIndices.length) {
        refillVerbShuffle()
    }
    return SPINNER_VERBS[shuffledVerbIndices[verbShufflePos++]]
}
...
function renderHeader() {
    ...
    // Tips are trusted authored markdown (some contain intentional `code`
    // spans). Don't escape — the whole header is rendered to entities, so
    // escaping would turn the tip's backticks into literal characters.
    const tipLine = tip ? `\n\n_did you know:_ ${tip}` : ""
    return `_${randomVerb()}..._${tipLine}`
}
...
function clearSessionSpinner(core, sessionId) {
    ...
    const { activeSpinner: _dropped, ...rest } = prev
    core.chatSessions = {
        ...core.chatSessions,
        [sessionId]: rest,
    }
...
async function sendSpinnerMessage(core, chatId, sessionId, threadId) {
    ...
    const headerHtml = applyOutputReplace(renderHeader())
    // Apply the General-bound guard before sending. This path bypasses
    // sendTextMessageToUser (it needs the messageId synchronously to
    // store activeSpinner.messageId), so we call the guard directly so
    // the prepended-header invariant still holds for unbound sessions.
    const guardedHeader = maybePrependGeneralHeader(
        headerHtml,
        chatId,
        threadId,
        { sessionId },
    ...
    // Render the markdown header to Telegram entities and send with no
    // parse_mode (see lib/pure/telegram-markdown.js). The spinner body is
    // full of code fences + arbitrary command output; entity rendering is
    // the only thing that survives that without mangling.
    const { text: hdrText, entities: hdrEntities } = renderMarkdownToEntities(guardedHeader)
    const opts = { format: "plain", entities: hdrEntities, silent: true }
    if (threadId != null) { opts.threadId = threadId }
    let sent
    try {
        sent = await core.bot.sendText(chatId, hdrText, opts)
    } catch (e) {
    ...
    recordOutboundMessage(core, {
        ...
        kind: "spinner",
        sessionId,
        text: guardedHeader.slice(0, 500),
        items: [],
        headerHtml,
    })
...
async function appendSpinnerItem(core, sessionId, rendered, ts, toolUseId) {
    ...
    const rawText = applyOutputReplace(renderSpinner(nextSpinner.headerHtml, nextSpinner.items))
    // Same General-bound guard as sendSpinnerMessage. We need the threadId
    // that the original spinner-send used; mirror it on the spinner state
    // so future edits target the same thread (and so the guard's "is this
    // landing in General?" check sees the same routing). Stored as
    // `threadId` on activeSpinner — see setSessionSpinner.
    const editThreadId = nextSpinner.threadId ?? null
    const text = maybePrependGeneralHeader(
        rawText,
        nextSpinner.chatId,
        editThreadId,
        { sessionId },
    ...
    // Render markdown → entities and edit with no parse_mode (see the send
    // path above and lib/pure/telegram-markdown.js).
    const { text: bodyText, entities: bodyEntities } = renderMarkdownToEntities(text)
    try {
        await core.bot.editText(
            nextSpinner.chatId,
            nextSpinner.messageId,
            bodyText,
            { format: "plain", entities: bodyEntities },
        )
    } catch (e) {
...
export async function applySpinnerPolicy(event, action, core) {
    ...
    // ── Start: queue-drained messages ─────────────────────────────────
    // When a queued message is drained (either from messageQueue on
    // session_register, or from pendingQueue on claude_hook_stop), the
    // event type is NOT chat_user_message, so the spinner start above
    // doesn't fire. We detect queue drains via the _queueDrain flag
    // on the deliver effect and create a spinner using the original
    // chatId/threadId from the queued entry.
    // ── Clear: agent finished its turn ────────────────────────────────
    // claude_hook_stop is the real end-of-turn signal. Clear here (NOT
    // on `reply`) so the spinner survives intermediate replies and keeps
    // showing tool activity for the whole turn. Done before the drain
    // below so a queued next-turn message gets a fresh spinner.
    if (event.type === "claude_hook_stop" && event.sessionId) {
        clearSessionSpinner(core, event.sessionId)
    }

    if ((event.type === "session_register" || event.type === "claude_hook_stop") && action?.effects) {
        const drainedDelivers = action.effects.filter(
            e => e?.type === "deliver_channel_event" && e._queueDrain
        )
        for (const deliver of drainedDelivers) {
            if (deliver.sessionId && deliver._chatId) {
                await sendSpinnerMessage(core, deliver._chatId, deliver.sessionId, deliver._threadId)
            }
        }
        // Don't return — fall through so other spinner policies can
        // still run for the same event if needed.
    }
    ...
    if (event.type === "claude_hook_pre_tool_use" || event.type === "claude_hook_post_tool_use") {
        ...
        // Process spinners for all sessions — each session's topic gets its own updates.
        const session = core.chatSessions?.[event.sessionId]
        // The spinner lives for the whole turn (created on the user
        // message, cleared on claude_hook_stop). An intermediate `reply`
        // no longer clears it, so a live spinner is always present while
        // the agent is doing tool work — just append to it.
