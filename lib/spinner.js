...
async function sendSpinnerMessage(core, chatId, sessionId) {
    if (!core.bot) {
    ...
    let sent
    try {
        sent = await core.bot.sendText(chatId, headerHtml, { format: "html" })
    } catch (e) {
...
async function appendSpinnerItem(core, sessionId, rendered, ts) {
    if (!core.bot) { return }
    const nextSpinner = mutateSpinnerItems(core, sessionId, (items) => {
        items.push({ rendered, ts: ts ?? Date.now() })
...
export async function applySpinnerPolicy(event, action, core) {
    ...
    if (event.type === "chat_user_message" && action?.effects) {
        ...
        const deliver = action.effects.find(e => e?.type === "deliver_channel_event")
        if (deliver?.sessionId) {
            await sendSpinnerMessage(core, event.chatId, deliver.sessionId)
        }
        return
    }

    // ── Append: Pre/PostToolUse hook on the focused session ────────
    if (event.type === "claude_hook_pre_tool_use" || event.type === "claude_hook_post_tool_use") {
        if (!event.sessionId) { return }
        if (core.chatState?.focusedSessionId !== event.sessionId) { return }
        ...
        await appendSpinnerItem(core, event.sessionId, rendered, event.ts)
        return
    }
