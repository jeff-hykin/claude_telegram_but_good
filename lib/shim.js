...
# Interactive mode: add --channels, wrap in dtach if available
CHANNELS_FLAG="--channels"
CHANNELS_VAL="plugin:telegram@claude-plugins-official"
...
if command -v dtach >/dev/null 2>&1; then
    ...
    exec dtach -c "$SOCK" -z "$REAL" $CHANNELS_FLAG "$CHANNELS_VAL" "$@"
else
    # No dtach — still add channels but run directly
    exec "$REAL" $CHANNELS_FLAG "$CHANNELS_VAL" "$@"
fi
`
}
