...
import { extname, basename, SEPARATOR, InputFile } from "../imports.js"
import { assertAllowedChat, loadAccess } from "./access.js"
import { STATE_DIR, INBOX_DIR, logMessage } from "./protocol.js"

const MAX_CHUNK_LIMIT = 4096
// Telegram Bot API hard limit: bots can send documents up to 50 MB through
// the public Bot API. Local Bot API servers can be configured higher, but
// we target the public service. https://core.telegram.org/bots/api#sending-files
        ...
        try {
            switch (name) {
                case "reply": {
                    ...
                    const parseMode = format === "html"
                        ? "HTML"
                        : format === "markdownv2" ? "MarkdownV2" : undefined
                    ...
                    for (const f of files) {
                        ...
                        if (st.size > TELEGRAM_MAX_DOCUMENT_BYTES) {
                            const sizeMb = (st.size / 1024 / 1024).toFixed(1)
                            throw new Error(`file too large for the Telegram Bot API: ${f} (${sizeMb}MB, Telegram caps bot uploads at 50MB)`)
                        }
                    }
                    ...
                    for (const f of files) {
                        ...
                        // esm.sh resolves grammy to its web build, whose
                        // InputFile.toRaw() does not handle string filesystem
                        // paths — it only knows Blob/URL/Uint8Array. Read the
                        // file ourselves and pass a Uint8Array so we work
                        // under both web and node grammy builds.
                        const bytes = Deno.readFileSync(f)
                        const input = new InputFile(bytes, basename(f))
                    ...
                    if (onReplyCallback) {
                        try { await onReplyCallback(text, chat_id) } catch (e) {
                            // onReplyCallback is best-effort — don't fail the reply if it throws
                            console.error("onReplyCallback failed:", e)
                        }
                    }
                ...
                case "edit_message": {
                    ...
                    const editParseMode = editFormat === "html"
                        ? "HTML"
                        : editFormat === "markdownv2" ? "MarkdownV2" : undefined
