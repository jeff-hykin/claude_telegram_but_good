...
export function isDaemonRunning() {
    try {
        ...
        // A missing PID_FILE is the NORMAL case when nothing is running
        // yet (first-time onboarding, post-stop state). Only log when
        // the error is something genuinely unusual — permission denied,
        // corrupt value, etc.
...
<dict>
  ...
  <string>${xmlEscape(paths.DAEMON_STDERR_FILE)}</string>
  <key>StandardOutPath</key>
  <string>${xmlEscape(paths.DAEMON_STDOUT_FILE)}</string>
...
// Backup connectivity watchdog (macOS only). A second launchd agent that
// fires hourly and restarts the main daemon if the Telegram heartbeat has
// gone stale. See event-generators/watchdog/connectivity-watchdog.js.
...
<dict>
  <key>Label</key>
  <string>${xmlEscape(paths.LAUNCHD_WATCHDOG_LABEL)}</string>
  <key>ProgramArguments</key>
  <array>
    <string>${xmlEscape(deno)}</string>
    <string>run</string>
    <string>-A</string>
    <string>${xmlEscape(paths.WATCHDOG_JS)}</string>
  </array>
  ...
  <true/>
  <key>StandardErrorPath</key>
  <string>${xmlEscape(paths.STATE_DIR)}/connectivity-watchdog.log</string>
  <key>StandardOutPath</key>
  <string>${xmlEscape(paths.STATE_DIR)}/connectivity-watchdog.log</string>
  <key>EnvironmentVariables</key>
  <dict>
...
export function installWatchdog() {
    ...
    try {
        ...
        Deno.writeTextFileSync(paths.LAUNCHD_WATCHDOG_PLIST_FILE, watchdogPlistContent())
        // Load ONLY if not already loaded, and NEVER unload. The watchdog
        // triggers `cbg restart`, which reaches restartService() ->
        // installWatchdog() — so the process running this may BE the
        // watchdog. A `launchctl unload` here would SIGTERM that running
        // job and leave the agent unloaded (launchd won't respawn an
        // unloaded agent), which is exactly what silently killed the
        // watchdog overnight on 2026-07-01. `load` on an already-loaded
        // agent is a harmless no-op. Trade-off: plist-content changes need
        // a manual reload — fine for this rarely-touched file.
        const listed = new Deno.Command("launchctl", {
            args: ["list", paths.LAUNCHD_WATCHDOG_LABEL],
            stdout: "null",
            stderr: "null",
        }).outputSync()
        if (!listed.success) {
            launchdExec(["load", paths.LAUNCHD_WATCHDOG_PLIST_FILE])
        }
    } catch (e) {
        dbg("DAEMON", "installWatchdog failed:", e)
    }
}

function removeWatchdog() {
    if (!IS_MACOS) { return }
    try { launchdExec(["unload", paths.LAUNCHD_WATCHDOG_PLIST_FILE]) } catch (e) { dbg("DAEMON", "unload watchdog:", e) }
    try { Deno.removeSync(paths.LAUNCHD_WATCHDOG_PLIST_FILE) } catch (e) { dbg("DAEMON", "remove watchdog plist:", e) }
}
