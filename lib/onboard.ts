...
import { ensureDtach, isDtachInstalled } from "./dtach.ts"
import { readConfig, setConfig, getBotToken } from "./config.ts"
...
export async function onboard(): Promise<void> {
  ...
  const claudeCheck = exec("which", ["claude"])
  ...
  const pluginSrcDir = join(import.meta.dirname ?? new URL(".", import.meta.url).pathname, "..")

  // Target symlink location
  const pluginDir = join(HOME, ".claude", "plugins", "marketplaces", "claude-plugins-official", "external_plugins", "telegram")

  // Create symlink
  try {
    // Remove existing (might be a directory or stale symlink)
  ...
  try {
    ...
    // Ensure channelsEnabled
    if (!settings.channelsEnabled) {
      settings.channelsEnabled = true
    }

    // Ensure plugin is in enabledPlugins
    const enabledPlugins = (settings.enabledPlugins as string[] | undefined) ?? []
  ...
  const checks = [
    ...
    { name: "plugin symlink", ok: (() => { try { Deno.statSync(pluginDir); return true } catch { return false } })() },
  ]
...
export function isOnboarded(): boolean {
  if (!isDtachInstalled()) return false
