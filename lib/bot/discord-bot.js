...
/**
 * Convert Telegram legacy-Markdown to Discord's Markdown dialect.
 *
 * Telegram legacy Markdown:
 *   *bold*           → Discord wants **bold**
 *   _italic_         → Discord accepts *italic* or _italic_ (keep as-is)
 *   `inline code`    → same in Discord
 *   ```code block``` → same in Discord
 *   [text](url)      → same in Discord
 *
 * Discord doesn't have a single-asterisk-for-bold; it uses ** for bold
 * and * or _ for italic. Telegram's *bold* must be promoted to **bold**
 * or it would render as italic in Discord. Underscore italics, code
 * spans, and code fences pass through unchanged. This is lossy in
 * edge cases (escaped asterisks) but preserves readability.
 */
function telegramMarkdownToDiscord(md) {
    // Promote *bold* → **bold**. The Telegram caller emits balanced
    // `*...*` pairs; we don't try to handle escaped backslash-asterisks
    // here (legacy Markdown's only escape) — the renderer is
    // best-effort, and the alternative (full parser) is overkill for a
    // demo adapter.
...
export class DiscordBot extends Bot {
     ...
     * Discord uses Markdown natively; Telegram legacy Markdown is
     * lossy-converted (mainly: *bold* → **bold**).
     */
    _toDiscordPayload(text, options = {}) {
        let content = String(text ?? "")
        if (options.format === "markdown") {
            content = telegramMarkdownToDiscord(content)
        }
    ...
    get supports() {
        return {
            ...
            markdownFormatting: true,   // lossy-converted from Telegram legacy Markdown
            fileDownload: true,
            threads: false,
        }
    }
