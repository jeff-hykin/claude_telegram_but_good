...
const { logTelegramTraffic, inboundEntryFromCtx } = await versionedImport("../telegram-traffic-log.js", import.meta)
...
export class TelegramBot extends Bot {
    ...
    constructor(config) {
        ...
        // Updated every time Grammy delivers a message. The watchdog
        // in main-server.js reads this to detect silent polling death.
        this._lastMessageAt = 0         // epoch ms, 0 = never received
        this._startedAt = 0             // epoch ms, set on start()
        this._pollingDied = false       // true if polling loop exited
        this._restartCount = 0          // auto-restart counter
    }
    ...
    async start() {
        ...
        // Log polling/middleware errors. Without this, grammy's default
        // handler swallows them, hiding bugs like silent polling-loop
        // exits after macOS sleep/wake (see topics/cbg/memory.md
        // "Telegram polling dies silently on laptop sleep").
        this._grammy.catch((err) => {
            const ts = new Date().toISOString()
            const e = err?.error ?? err
            const stack = e?.stack ?? ""
            console.error(`[TG-POLL ${ts}] grammy error:`, e?.message ?? String(e), stack ? `\n${stack}` : "")
        })

        const dispatch = (ctx) => {
            this._lastMessageAt = Date.now()
            const entry = inboundEntryFromCtx(ctx)
            if (entry) { logTelegramTraffic(entry) }
            if (this._onMessage) {
        ...
        // The bot's own membership changing — fires when someone adds it
        // to a group, which is when that group's listening session spawns.
        this._grammy.on("my_chat_member", dispatch)

        this._startedAt = Date.now()
        this._pollingDied = false
        ...
        return await new Promise((resolve, reject) => {
            ...
            ;(async () => {
                try {
                    ...
                    // If we get here, polling loop exited cleanly after
                    // startup — this should NOT happen during normal
                    // operation. Flag it so the watchdog can restart.
                    if (settled) {
                        this._pollingDied = true
                        this._log("ALERT: Grammy polling loop exited cleanly — bot is no longer receiving messages")
                    }
                } catch (e) {
                    if (!settled) {
                        settled = true
                        ...
                        // Polling loop crashed AFTER successful startup.
                        this._pollingDied = true
                        this._log(`ALERT: Grammy polling loop crashed — ${e?.message ?? String(e)}`)
                    }
                }
            })()
        })
    ...
    /**
     * Restart the Grammy polling loop. Called by the watchdog when
     * polling death is detected. Returns true on success.
     */
    async restart() {
        this._log(`restarting Grammy polling (restart #${this._restartCount + 1})`)
        try {
            // Stop the old instance if it's still technically alive
            if (this._grammy) {
                try { await this._grammy.stop() } catch (_e) { /* best effort */ }
            }
            this._started = false
            this._grammy = null
            this._pollingDied = false
            this._restartCount++
            await this.start()
            this._lastMessageAt = Date.now()  // reset silence timer so watchdog doesn't immediately re-trigger
            this._log("Grammy polling restarted successfully")
            return true
        } catch (e) {
            this._log(`Grammy restart failed: ${e?.message ?? String(e)}`)
            return false
        }
    }

    /**
     * Health info for the watchdog. Returns an object describing
     * whether the bot appears healthy.
     */
    get pollingHealth() {
        return {
            ...
            pollingDied: this._pollingDied,
            restartCount: this._restartCount,
            msSinceLastMessage: this._lastMessageAt > 0
    ...
    /**
     * getMe is the cheapest authenticated round-trip Telegram offers, so
     * it answers "is this daemon still able to reach Telegram?" without
     * needing anyone to send a message.
     */
    async ping() {
        if (!this._grammy) { return false }
        try {
            await this._grammy.api.getMe()
            return true
        } catch (e) {
            this._log(`ping failed: ${e?.message ?? String(e)}`)
            return false
        }
    }
            ...
            while (this._queue.length > 0) {
                ...
                try {
                    const result = await Promise.race([
                        entry.fn(),
                        new Promise((_, rej) => setTimeout(() => rej(new Error(`Telegram API timeout: ${entry.label} took >30s`)), 30_000)),
                    ])
                    this._stats.sent++
                    this._recordSend()
     ...
     *
     *   format: "markdown" → parse_mode: "Markdown" (legacy Markdown)
     *   format: "html"     → parse_mode: "HTML" (deprecated, accepted for
     *                        any legacy callers still passing it)
    ...
    _toGrammyOptions(options) {
        const opts = { ...(options ?? {}) }
        if (opts.format === "markdown") { opts.parse_mode = "Markdown" }
        else if (opts.format === "markdownv2") { opts.parse_mode = "MarkdownV2" }
        else if (opts.format === "html") { opts.parse_mode = "HTML" }
        delete opts.format
    ...
    async sendText(chatId, text, options) {
        ...
        // DEBUG HACK (Jeff 2026-04-28): if a message is heading to the CC
        // group's General topic (chatId == ccChatId AND no threadId),
        // dump the JS call stack to the daemon log so we can identify
        // every code path that bypasses the outbound-layer header guard.
        // Cheap (one file read + one Error allocation per send) and
        // non-blocking — the actual send proceeds normally.
        _traceIfGeneralBound("sendText", chatId, options?.threadId, text)
        logTelegramTraffic({
            direction: "out", kind: "text",
            chatId: String(chatId), threadId: options?.threadId ?? null,
            text, format: options?.format ?? null,
        })
    ...
    async sendFile(chatId, filePath, options = {}) {
        ...
        _traceIfGeneralBound("sendFile", chatId, options?.threadId, options?.caption ?? filePath)
        logTelegramTraffic({
            direction: "out", kind: "file",
            chatId: String(chatId), threadId: options?.threadId ?? null,
            filePath, filename: options?.filename ?? null,
            caption: options?.caption ?? null, format: options?.format ?? null,
        })
        ...
        const { filename, caption, caption_entities, format, threadId } = options
        const opts = {}
        if (caption) {
            opts.caption = caption
            // Explicit caption_entities (from our markdown→entities renderer)
            // take precedence and mean NO parse_mode. Otherwise fall back to
            // the format→parse_mode mapping for any legacy callers.
            if (Array.isArray(caption_entities)) {
                opts.caption_entities = caption_entities
            } else if (format === "markdown") { opts.parse_mode = "Markdown" }
            else if (format === "markdownv2") { opts.parse_mode = "MarkdownV2" }
            else if (format === "html") { opts.parse_mode = "HTML" }
        }
        if (threadId != null) {
    ...
    async editText(chatId, messageId, text, options) {
        ...
        _traceIfGeneralBound("editText", chatId, options?.threadId, text)
        logTelegramTraffic({
            direction: "out", kind: "edit",
            chatId: String(chatId), threadId: options?.threadId ?? null,
            messageId: String(messageId), text, format: options?.format ?? null,
        })
    ...
    async pinMessage(chatId, messageId, options) {
        this._assertStarted("pinMessage")
        return this._enqueue("low", "pinMessage", async () => {
            try {
                await this._grammy.api.pinChatMessage(chatId, Number(messageId), {
                    disable_notification: options?.silent !== false,
                })
                return true
            } catch (e) {
                console.error(`[TG-BOT] pinChatMessage failed (chat=${chatId} msg=${messageId}): ${e?.description ?? e?.message ?? e}`)
                return false
            }
        })
    }
    ...
    async unpinMessage(chatId, messageId) {
        this._assertStarted("unpinMessage")
        return this._enqueue("low", "unpinMessage", async () => {
            try {
                await this._grammy.api.unpinChatMessage(chatId, { message_id: Number(messageId) })
                return true
            } catch (e) {
                console.error(`[TG-BOT] unpinChatMessage failed (chat=${chatId} msg=${messageId}): ${e?.description ?? e?.message ?? e}`)
                return false
            }
        })
    }
    ...
    async react(chatId, messageId, emoji) {
        ...
        logTelegramTraffic({
            direction: "out", kind: "reaction",
            chatId: String(chatId), messageId: String(messageId), emoji: emoji ?? null,
        })
    ...
    async answerCallback(queryId, text) {
        ...
        logTelegramTraffic({
            direction: "out", kind: "callback_answer",
            queryId: String(queryId), text: text ?? null,
        })
    ...
    get supports() {
        return {
            reactions: true,
            inlineButtons: true,
            htmlFormatting: false, // house rule (post-2026-05-16): we use legacy Markdown
...
// ── DEBUG HACK: General-bound send tracer ─────────────────────────────
//
// Jeff 2026-04-28: messages keep landing in the CC group's General
// topic without identifying info despite the outbound-layer guard at
// lib/effects/telegram-outbound.js. That guard only catches sends
// routed through `sendTextMessageToUser` / `sendFileToUser`. There are
// other call sites (`lib/spinner.js`, `lib/access.js`, possibly more)
// that hit `core.bot.sendText` directly and bypass it.
//
// This hook fires at the absolute wire layer, just before the Grammy
// API call. Any send heading to General (chatId == ccChatId AND no
// threadId) gets its full call stack dumped to the daemon log via
// dbg("TG-OUT-TRACE", ...). After deploying, send a few messages, then
// `grep TG-OUT-TRACE ~/.local/share/cbg/state/daemon.stderr.log` to
// see exactly which code paths bypass the header guard.
//
// Inline file read so we don't have to refactor module imports for a
// debug hack — synchronous, ~once per send, negligible overhead vs the
// HTTPS round-trip that follows. Logs but never throws.
let _accessCacheJson = null
let _accessCacheCheckedAt = 0
function _readCommandCenterChatId() {
    const now = Date.now()
    // Cheap throttle — re-read at most once per second.
    if (now - _accessCacheCheckedAt < 1000) { return _accessCacheJson?.commandCenterChatId ?? null }
    _accessCacheCheckedAt = now
    try {
        const home = Deno.env.get("HOME") ?? ""
        const cbgDir = Deno.env.get("CBG_DIR") ?? `${home}/.local/share/cbg`
        const accessPath = `${cbgDir}/state/access.json`
        const raw = Deno.readTextFileSync(accessPath)
        _accessCacheJson = JSON.parse(raw)
    } catch (_e) {
        _accessCacheJson = null
    }
    return _accessCacheJson?.commandCenterChatId ?? null
}

function _traceIfGeneralBound(label, chatId, threadId, text) {
    try {
        const ccChatId = _readCommandCenterChatId()
        if (!ccChatId) { return }
        if (String(chatId) !== String(ccChatId)) { return }
        if (threadId != null) { return }
        const preview = String(text ?? "").replace(/\s+/g, " ").slice(0, 80)
        const stack = (new Error().stack ?? "").split("\n").slice(1).join("\n")
        // Use console.error directly — avoids importing dbg() from a file
        // that isn't versioned. Stderr is captured into daemon.stderr.log.
        console.error(`[TG-OUT-TRACE ${new Date().toISOString()}] ${label} bound for General (chatId=${chatId}, no threadId): "${preview}"\n${stack}`)
    } catch (_e) {
        // Tracing must never break a real send.
    }
}
