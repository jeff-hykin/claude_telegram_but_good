...
export class TelegramBot extends Bot {
     ...
     * malformed name doesn't drop the menu. Replacement-style: this
     * call OVERWRITES whatever Telegram currently has registered, so
     * stale commands from older versions of the bot (or manual
     * BotFather edits) get cleaned up on every boot.
     */
    async setCommands(commands) {
        ...
        try {
            await this._grammy.api.setMyCommands(filtered)
            return true
        } catch (_e) {
            return false
        }
