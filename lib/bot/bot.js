...
//   - async pinMessage(chatId, messageId, options?) → boolean
//   - async unpinMessage(chatId, messageId) → boolean
//
...
export class Bot {
    ...
    /**
     * Round-trip the platform's API to prove the connection actually
     * works right now. Unlike inbound traffic this is independent of
     * whether anyone is talking to the bot, so a quiet night can't be
     * mistaken for a dead daemon. Adapters that can't probe return true
     * rather than reporting a failure they didn't observe.
     */
    ...
    async pinMessage(_chatId, _messageId, _options) { return false }
    async unpinMessage(_chatId, _messageId) { return false }
