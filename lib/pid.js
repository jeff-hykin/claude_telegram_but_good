...
export function findClaudePidStrict(startPid) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        //
        // The optional leading `_` covers cbg's shim-setup rename: the
        // real claude binary is moved to `_claude_before_cbg` so cbg's
        // own `claude` script can sit on PATH. Without this, hooks on
        // Linux (where the rename sticks as argv[0]) fail to resolve
        // the claude ancestor and every hook event gets tagged with
        // UNKNOWN_CLAUDE_PID.
        if (/^(?:\S*\/)?_?claude/i.test(args)) {
