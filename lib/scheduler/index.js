...
export function computeNextFire(rule, from) {
    ...
    // Set dtstart slightly before `from` so the first occurrence of the
    // pattern lands AFTER `from`. Without this, count=1 rules are
    // immediately exhausted because their single occurrence is AT
    // dtstart, and r.after(from, exclusive) skips it.
    const dtstart = new Date(phantomFrom.getTime() - 1000)
    const opts = toRRuleOptions(rule, dtstart)
    const r = new RRule(opts)
    const phantomNext = r.after(phantomFrom, false /* inclusive=false */)
