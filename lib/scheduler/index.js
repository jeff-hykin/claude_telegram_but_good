...
/**
 * Grace period before a fire that was missed while the daemon was down
 * is replayed. Long enough for the daemon to finish booting and for a
 * shim to reconnect, short enough that a restart doesn't cost a run.
 */
export const MISSED_FIRE_DELAY_MS = 30 * 1000

/**
 * Where a timer should be re-armed after a restart, given the fire time
 * that was persisted before the daemon went down.
 *
 * Re-arming from `now` instead of the persisted time is what let a chain
 * of daemon restarts push a schedule indefinitely into the future — six
 * restarts on a 3h interval starved it for 10h. So a persisted fire time
 * that is still ahead of us is honored exactly, and one we slept through
 * is replayed shortly rather than skipped.
 *
 * Returns null when there's nothing usable to resume from, in which case
 * the caller should fall back to computing from the rule.
 */
export function resolveResumeFire(resumeAt, rule, now = new Date()) {
    if (!resumeAt) { return null }
    const resume = new Date(resumeAt)
    if (isNaN(resume.getTime())) { return null }
    const at = resume.getTime() > now.getTime()
        ? resume
        : new Date(now.getTime() + MISSED_FIRE_DELAY_MS)
    // A resume time past the rule's own end would fire a run the rule
    // says should never happen; fall back so the caller retires it.
    if (rule?.until && at.getTime() > new Date(rule.until).getTime()) {
        return null
    }
    return { at, missed: resume.getTime() <= now.getTime() }
}
