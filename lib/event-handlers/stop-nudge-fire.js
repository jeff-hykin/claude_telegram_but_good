...
// Hard floor on task-report nudge cadence. set_timer has no cancellation,
// so each Stop stacks another timer and several can come due together; this
// gate (checked against the task's lastNudgeAt at fire time) guarantees we
// never send the "[automated reminder] … no report.md" nudge more than once
// per 20 minutes, no matter how many timers pile up.
const MIN_TASK_NUDGE_INTERVAL_MS = 20 * 60 * 1000

function chatIdLine(chatId) {
    return chatId ? ` The Telegram chat_id for this conversation is ${chatId} — pass it to the reply tool.` : ""
}

function replyNudgeText(chatId) {
    return "[automated reminder] You received a Telegram message but haven't replied yet. Please call the telegram reply tool now to respond to the user." + chatIdLine(chatId)
}

function reportNudgeText(taskId, chatId) {
...
export default function handle(event, core) {
    ...
    if (nudgeType === "reply") {
        ...
        const replyChatId = chatId ?? session.lastInbound?.chatId ?? null
        return {
            stateChanges: {
                chatSessions: {
            ...
            effects: [{
                ...
                sessionId,
                text: replyNudgeText(replyChatId),
            }],
        }
    }
    ...
    if (nudgeType === "taskReport") {
        ...
        // Rate-limit: never nudge more than once per 20 min, even if several
        // stacked timers come due at once (set_timer can't be cancelled).
        const now = event.ts ?? Date.now()
        if (task.lastNudgeAt && (now - task.lastNudgeAt) < MIN_TASK_NUDGE_INTERVAL_MS) {
            const agoMin = Math.round((now - task.lastNudgeAt) / 60000)
            dbg("STOP-NUDGE", `task nudge for ${taskId}: last nudge ${agoMin}m ago (<20m), skipping`)
            return null
        }
            ...
            effects: [
                {
                    type: "send_text_to_claude",
                    sessionId,
                    text: reportNudgeText(taskId, chatId),
                },
