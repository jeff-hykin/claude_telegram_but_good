...
export default function handle(event, core) {
    ...
    if (status === "certified") {
        ...
        // <silent> sentinel: monitor-style tasks can write response.md starting
        // with "<silent>" to suppress the Telegram message + attachments on
        // clean runs. Everything after the sentinel stays in response.md for
        // the run record but is never sent.
        if (responseText && responseText.startsWith("<silent>")) {
            dbg("SCHED-DONE", `${scheduleTaskId} response.md begins with <silent> — suppressing Telegram send`)
        } else {
