...
const { matchHooks, replyBlockText, addHook } = await versionedImport("../memory-hooks.js", import.meta)
...
export default function handle(event, core) {
    ...
    switch (toolName) {
        ...
        case "create_memory_hook":           return handleCreateMemoryHook(event, core)
...
function handleReply(event, core) {
    ...
    // Listen mode: the session reads this chat but is not allowed to post
    // in it. Refusing here (rather than in the agent's prompt) is the
    // whole point — a bot sitting in someone else's group must stay quiet
    // even when the model is convinced it should speak.
    const listenReason = listenBlockReason(core.chatSessions?.[sessionId], chatId)
    if (listenReason) {
        dbg("CHANNEL", `listen mode blocked a reply from ${sessionId} to ${chatId}`)
        return replyError(_conn, requestId, listenReason)
    }

    // /tldr mode: if a max length is set and this reply's text exceeds it,
    // reject the reply (nothing is sent to the user) and tell the agent to
    // shorten it. The agent sees this as a tool error and retries concisely.
    const tldrMaxChars = core.chatState?.tldrMaxChars
    if (typeof tldrMaxChars === "number" && tldrMaxChars > 0 && text.length > tldrMaxChars) {
        return replyError(
            _conn,
            requestId,
            `Too long — ${text.length} chars (limit ${tldrMaxChars}). Respond like a human texting, not an essay. ` +
            `Think about what an actual person would say. E.g. to "what's the weather going to be like today?" a human says ` +
            `"Rainy" or "50% chance of rain" — NOT "At 9am there's a 38% chance of rainfall, though total volume is small; ` +
            `low of 60 so bring a jacket, high of 90." Answer the question directly in as few words as possible, then call reply again.`,
        )
    }

    // Memory hooks: the agent is about to say something it has a standing
    // note about. Catching it here rather than after the send is the point
    // — the user never sees the uncorrected version. Each hook fires at
    // most once per turn (claude-hook-stop.js clears the set), so a hook
    // whose note doesn't apply costs one retry, not a loop.
    const alreadyFired = core.chatSessions?.[sessionId]?.memoryHooksFired ?? []
    const trippedHooks = matchHooks(text, "agent", alreadyFired)
    if (trippedHooks.length > 0) {
        dbg("CHANNEL", `memory hooks blocked a reply from ${sessionId}: ${trippedHooks.map((hook) => hook.pattern).join(", ")}`)
        return {
            ...replyError(_conn, requestId, replyBlockText(trippedHooks)),
            stateChanges: {
                chatSessions: {
                    [sessionId]: {
                        memoryHooksFired: [...alreadyFired, ...trippedHooks.map((hook) => hook.id).filter(Boolean)],
                    },
                },
            },
        }
    ...
    // We need topic/CC info BEFORE building the header so we can detect
    // when the message would land in the CC group's General topic (no
    // thread_id) and emit a richer header with full session context —
    // otherwise the user has no anchor for "who is this from?".
    ...
    // Will the un-threaded copy land in the CC group's General topic?
    // (CC group with no topic-thread to route to.) When true, we boost
    // the header to include topic/cwd/branch/pid so the user can tell
    // which agent is talking — there's no thread context to anchor on.
    const dmCopyWillLandInGeneral = replyIsToCommandCenter && !topicThreadId
    ...
    const headeredText = makeChatHeader(sessionId, core, args.format, { verbose: dmCopyWillLandInGeneral }) + text
    ...
    // Only the first file gets pinned — pinning every attachment of a
    // multi-file reply would bury the chat's pin list.
    const pinRequested = args.pin === true
    ...
    // Write to topic inbox for persistent history
    const replyTopicName = sessionTopicName(sessionId, core)
    if (replyTopicName) {
        effects.push({
            type: "write_inbox",
            address: `topic:${replyTopicName}`,
            message: {
                ts,
                from: { type: "session", sessionId },
...
/**
 * Inbox-only delivery path used when the target is either an explicit
 * `inbox:<addr>` or a bare address that didn't resolve to any live
 * session. Writes the message to the inbox, wakes up any parked
 * ask_sync waiter, and logs the inter-session hop.
 */
function inboxOnlyDelivery(address, text, fromSessionId, core, _conn, requestId) {
    const fromTopic = sessionTopicName(fromSessionId, core) || "_unbound"
    const inboxMsg = {
        ts: Date.now(),
        from: { type: "session", sessionId: fromSessionId, topicName: fromTopic },
        text,
    }
    return {
        stateChanges: {},
        effects: [
            ...replyOk(_conn, requestId, `message written to inbox "${address}" (no live session)`),
            { type: "write_inbox", address, message: inboxMsg },
            { type: "notify_inbox_waiter", address, message: inboxMsg },
            {
                type: "log_inter_session_message",
                entry: {
                    ts: new Date().toISOString(),
                    from: { sessionId: fromSessionId, topicName: fromTopic },
                    to: { inboxAddress: address },
                    text,
                    source: "tell_session_inbox",
                },
                topicNames: [fromTopic],
            },
        ],
    }
}
...
function handleTellSession(event, core) {
    ...
    if (!address) {
        return replyError(_conn, requestId, "tell_session: missing target (session:<id>, topic:<name>, title:<sub>, inbox:<addr>, cbg:<x>, or bare fuzzy)")
    }
    if (!text) {
        return replyError(_conn, requestId, "tell_session: missing text")
    }

    const resolved = resolveTargetPrefixed(address, core)
    const { mode } = parseTargetAddress(address)

    // inbox:<addr> — skip session resolution, write straight to inbox.
    if (resolved.inbox) {
        return inboxOnlyDelivery(resolved.inbox, text, sessionId, core, _conn, requestId)
    }

    // Resolution failed. For explicit modes (session:/topic:/title:), that's
    // an error — we don't silently write to some inbox named the raw string.
    // For auto mode (bare string or cbg:<x>), fall back to inbox-only using
    // the unwrapped value so legacy `tell_session target="my_cli"` keeps
    // writing to `my_cli` inbox.
    if (resolved.error) {
        if (mode === "auto") {
            const { value } = parseTargetAddress(address)
            return inboxOnlyDelivery(value, text, sessionId, core, _conn, requestId)
        }
        return replyError(_conn, requestId, `tell_session: ${resolved.error}`)
    }
...
function handleCreateMemoryHook(event, _core) {
    const { args = {}, requestId, _conn } = event
    const result = addHook({ pattern: args.pattern, hint: args.hint, on: args.on })
    if (result.error) {
        return replyError(_conn, requestId, `create_memory_hook: ${result.error}`)
    }
    dbg("CHANNEL", `memory hook ${result.updated ? "updated" : "created"}: ${result.hook.pattern}`)
    return {
        stateChanges: {},
        effects: replyOk(_conn, requestId, JSON.stringify({
...
function handleStartLongWellDefinedTask(event, core) {
    ...
    const agentInstructions = [
        ...
        ``,
        `Status command for the user: /task_status_${taskId}`,  // plain text prompt to Claude, not Telegram
    ].join("\n")
    ...
    const effects = [
        ...
        // since we're skipping that handler entirely.
        {
            ...
            chatId,
            text:
                `Agent started long task ${taskId}:` +
                `\n\n${title}` +
                `\n\n` +
 ...
 * Default is plain text (no parse_mode). Markdown uses parse_mode: "Markdown"
 * (legacy Markdown, see CLAUDE.md). HTML is accepted as a back-compat
 * value — existing Claude Code sessions cached the old MCP tool schema
 * at startup, which had `enum: ["text", "html", "markdownv2"]`, and
 * cannot pass "markdown" until they restart. Keeping the HTML path so
 * those sessions render formatted output instead of silently degrading
 * to plain text.
 */
function buildFormatOptions(format) {
    // Explicit plain-text escape hatch.
    ...
    // Everything else — INCLUDING an omitted format — renders as markdown.
    // Agents constantly write markdown but forget to set format:"markdown",
    // so the old plain-text default left their `*bold*`/`_italic_`/`` `code` ``
    // showing as literal markers. Defaulting to markdown is safe now: we
    // render it to explicit Telegram entities ourselves (lib/pure/telegram-
    // markdown.js) — never 400s, and unmatched/intraword markers degrade to
    // literal text. "markdownv2" maps here too (nothing writes real V2).
 ...
 * so we don't render a redundant `/chat_CalmLion (CalmLion)`. In Markdown
 ...
 */
function makeChatHeader(sessionId, core, format, opts = {}) {
    if (!sessionId) { return "" }
    const session = core?.chatSessions?.[sessionId]
    const rawTitle = typeof session?.title === "string" ? session.title : ""
    const hasDistinctTitle = rawTitle.length > 0 && rawTitle !== sessionId

    // Verbose mode: the message is going to land in the CC group's
    // General topic (no topic thread anchoring it). The user has no
    // visual context for which agent is talking, so dump everything we
    // know. Keeps the same /chat_<id> handle on the first line so the
    // tap-to-route shortcut still works.
    if (opts.verbose) {
        const cc = core?.chatState?.commandCenter ?? {}
        const threadId = cc.topicMap?.[sessionId] ?? null
        const topicName = threadId ? cc.topicNames?.[threadId] : null
        ...
        if (format === "markdown") {
            const lines = [
                `*/chat_${sessionId}*${hasDistinctTitle ? ` _${esc(rawTitle)}_` : ""}`,
            ]
            if (topicName) { lines.push(`topic: \`${esc(topicName)}\``) }
            if (cwd) { lines.push(`cwd: \`${esc(cwd)}\``) }
            if (gitBranch) { lines.push(`branch: \`${esc(gitBranch)}\``) }
            if (pid) { lines.push(`pid: ${pid}`) }
            lines.push("⚠️ _landed in General — no topic thread bound_")
            return lines.join("\n") + "\n\n"
        }
        const lines = [
            `/chat_${sessionId}${hasDistinctTitle ? ` (${rawTitle})` : ""}`,
        ]
        if (topicName) { lines.push(`topic: ${topicName}`) }
        if (cwd) { lines.push(`cwd: ${cwd}`) }
        if (gitBranch) { lines.push(`branch: ${gitBranch}`) }
        if (pid) { lines.push(`pid: ${pid}`) }
        lines.push("(landed in General — no topic thread bound)")
        return lines.join("\n") + "\n\n"
    }

    if (format === "markdown") {
        const titlePart = hasDistinctTitle ? ` (_${esc(rawTitle)}_)` : ""
