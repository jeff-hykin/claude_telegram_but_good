...
function buildFormatOptions(format) {
    ...
    if (format === "markdownv2") {
        // Map V2 → legacy Markdown. Agents whose cached MCP schema only
        // exposes "markdownv2" still write legacy-MD-syntax bodies
        // (asterisk-bold, bare dots/parens), and Telegram's V2 parser
        // rejects all of that. Treating the request as legacy renders
        // correctly. If a future agent genuinely needs V2, add the V2
        // value as a separate format string.
        return { parse_mode: "Markdown" }
    }
    return {}
}
