...
function handleTellSession(event, core) {
    ...
    // If no live session found, try inbox-only delivery.
    // This allows agents to send replies to CLI inboxes that have
    // no running session (e.g. cbg ask --from <inbox>).
    if (error) {
        const fromTopic = sessionTopicName(sessionId, core) || "_unbound"
        const inboxMsg = {
            ts: Date.now(),
            from: { type: "session", sessionId, topicName: fromTopic },
            text,
        }
        return {
            stateChanges: {},
