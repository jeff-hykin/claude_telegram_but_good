...
function handleWatchFile(event, core) {
    ...
    // Capture the initial state (mtime or non-existence).
    let initialMtime = null
    try {
        initialMtime = Deno.statSync(filePath).mtime?.getTime() ?? null
    } catch {
        // File doesn't exist yet — watch for creation.
    }

    // Poll via a timer chain. Each tick checks if the file changed.
    return {
        stateChanges: {},
        effects: [
            ...replyOk(_conn, requestId, JSON.stringify({ id, watching: filePath, timeout: `${timeoutSec}s` })),
            {
                type: "set_timer",
                delayMs: 2000,
                event: {
                    type: "agent_file_watch_tick",
                    sessionId,
                    watchId: id,
                    filePath,
                    message,
...
function handleTellSession(event, core) {
    ...
    if (!targetId) {
        return replyError(_conn, requestId, "tell_session: missing target_session_id")
    }
    if (!text) {
    ...
    if (!target?._conn) {
        return replyError(_conn, requestId, `tell_session: session "${targetId}" not found or disconnected`)
    }

    return {
        stateChanges: {},
        effects: [
            ...replyOk(_conn, requestId, `message delivered to ${targetId}`),
            {
                type: "deliver_channel_event",
                sessionId: targetId,
                content: `[from session ${sessionId}]: ${text}`,
                meta: { source: "tell_session", fromSession: sessionId },
            },
        ],
    }
}
