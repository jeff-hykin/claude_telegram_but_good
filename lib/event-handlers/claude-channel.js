...
function makeChatHeader(sessionId, core, format, opts = {}) {
    ...
    if (opts.verbose) {
        ...
        if (format === "html") {
            const lines = [
                `<b>/chat_${sessionId}</b>${hasDistinctTitle ? ` <i>${esc(rawTitle)}</i>` : ""}`,
            ]
            if (topicName) { lines.push(`topic: <code>${esc(topicName)}</code>`) }
            if (cwd) { lines.push(`cwd: <code>${esc(cwd)}</code>`) }
            if (gitBranch) { lines.push(`branch: <code>${esc(gitBranch)}</code>`) }
            if (pid) { lines.push(`pid: ${pid}`) }
            lines.push("⚠️ <i>landed in General — no topic thread bound</i>")
            return lines.join("\n") + "\n\n"
        }
        const lines = [
