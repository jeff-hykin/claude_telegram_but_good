...
function handleReply(event, core) {
    ...
    // Build replyTo objects for both delivery paths.
    const dmReplyTo = makeReplyTo({ chatId, threadId: null, setBy: "claude-channel:reply" })
    const topicReplyTo = (topicThreadId && ccChatId)
        ? makeReplyTo({ chatId: ccChatId, threadId: Number(topicThreadId), setBy: "claude-channel:reply-topic" })
        : null
    ...
    if (Array.isArray(args.files) && args.files.length > 0) {
        for (let i = 0; i < args.files.length; i++) {
            ...
            if (topicReplyTo && (outputMode === "group" || outputMode === "both")) {
                effects.push({
                    type: "send_file_to_user",
                    replyTo: topicReplyTo,
                    filePath,
                    filename,
        ...
        if (topicReplyTo && (outputMode === "group" || outputMode === "both")) {
            effects.push({
                ...
                replyTo: topicReplyTo,
                text: text,
                options: buildFormatOptions(args.format),
                recordAs: { ...recordAs, kind: "topic_mirror" },
            })
        }
    }
...
function handleStartLongWellDefinedTask(event, core) {
    ...
    const effects = [
        ...
        {
            type: "send_text_to_user",
            replyTo: makeReplyTo({ chatId, threadId: null, setBy: "claude-channel:start-long-task" }),
            text:
