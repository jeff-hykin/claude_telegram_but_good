...
function handleReply(event, _core) {
    ...
    // carries the originating sessionId, and telegram-user.js reads
    ...
    // Agent is delivering its final response — freeze the session's
    // spinner so subsequent hooks start a new one instead of mutating
    // the now-settled progress message.
    if (sessionId) {
        effects.push({ type: "clear_session_spinner", sessionId })
    }
    ...
    // the worker has responded since the last inbound. Resetting
    // `nudgedForInbound` is redundant here (a reply means we're caught up)
    // but keeps the flag consistent with the "worker replied" signal.
    const stateChanges = sessionId
        ? {
            chatSessions: {
                [sessionId]: {
                    lastOutboundAt: ts,
