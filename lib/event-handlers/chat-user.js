...
export default async function handle(event, core) {
    ...
    function wrap(action) {
        ...
        // Connectivity heartbeat: rewrite the timestamp on every message
        // we actually process from an allowed sender. The backup launchd
        // watchdog restarts the daemon if this goes stale (see
        // event-generators/watchdog/connectivity-watchdog.js). `isAllowed`
        // is assigned below before any path that calls wrap.
        const connectivityEffects = isAllowed
            ? [{ type: "write_file", path: paths.LAST_CONNECTIVITY_FILE, content: String(Date.now()) }]
            : []
        return {
            ...action,
            effects: [...(action?.effects ?? []), ...connectivityEffects],
            stateChanges: {
                ...prev,
                specialData: {
                    ...prevSpecial,
