...
const { makeReplyTo, sendEffect } = await versionedImport("../pure/reply-to.js", import.meta)
...
export default async function handle(event, core) {
    ...
    // ── Topic-based session resolution (command center groups) ──────
    // In command center mode, resolve the target session from the
    // topic's threadMap BEFORE any command dispatch. This ensures
    // /task, /cancel, plain text, and all other commands route to the
    // topic's session — never the globally focused session. For DM
    // chats, topicSessionId is null and focus-based routing applies.
    let topicSessionId = null
    if (isCommandCenter && event.threadId) {
        ...
        if (mappedSession && core.chatSessions?.[mappedSession]) {
            topicSessionId = mappedSession
            dbg("CHAT-USER", `command center topic routing: thread ${threadKey} → session ${mappedSession}`)
        } else if (!mappedSession) {
            // Orphan topic — no session bound. Let commands through
            // (they may not need a session), but mark for plain-text
            // rejection below.
            topicSessionId = null
        } else if (mappedSession === core.chatState?.pendingFocusId) {
            // Session is spawning but hasn't registered yet. For
            // plain text and /task, queue the message. For other
            // commands (/peek, /status, etc.), let them through.
            if (!isCommand) {
                dbg("CHAT-USER", `thread ${threadKey} → ${mappedSession} (pending, queueing message)`)
                const messageQueue = [...(core.chatState?.messageQueue ?? [])]
                messageQueue.push({
                    content: event.text || "",
                    meta: { source: "queued-for-pending", chatId, threadId: event.threadId },
                ...
                return wrap({
                    ...
                    effects: [{
                        type: "send_text_to_user",
                        chatId,
                        text: `Session <code>${esc(mappedSession)}</code> is still starting up — message queued for delivery.`,
                        options: { parse_mode: "HTML", message_thread_id: Number(event.threadId) },
                    }],
                })
            }
            topicSessionId = null  // let commands through without a session
        } else {
            // Stale mapping — session disconnected. For plain text, error.
            // For commands, let them through (some don't need a session).
            if (!isCommand) {
    ...
    // Effective session: topic session in groups, focused session in DMs.
    const effectiveSessionId = topicSessionId ?? core.chatState?.focusedSessionId ?? null

    // Stamp replyTo on the event so all downstream handlers and commands
    // know where to send responses. This is the SINGLE source of truth
    // for outbound message routing.
    event.replyTo = makeReplyTo(event, "chat-user")

    // 1.5 Attachment handling. Runs BEFORE the command-regex dispatches
    // so attachments always go to the topic/focused session instead of
    // being parsed as slash commands (their synthesized text starts with "(").
    if (event.attachment) {
    ...
    // 5. Plain text → deliver to the topic's session (command center)
    // or focused session (DM). Topic routing already happened above;
    // topicSessionId is set if we're in a command center topic with a
    // live session. Pending/stale/orphan cases were already handled
    // above for non-command messages.
    //
    ...
    // In command center with no topic session, reject plain text
    if (isCommandCenter && event.threadId && !topicSessionId) {
        return wrap({
            stateChanges: {},
            effects: [{
                type: "send_text_to_user",
...
function handleClose(event, core, targetId) {
    ...
    if (!target) {
        return reply(event.replyTo, `Session <code>${esc(targetId)}</code> not found.`)
    }
...
/**
 * Build a simple reply Action. Accepts either:
 *   reply(replyTo, text)           — new replyTo-based signature
 *   reply(chatId, text, threadId)  — legacy signature (migration compat)
 */
function reply(chatIdOrReplyTo, text, threadId) {
    if (typeof chatIdOrReplyTo === "object" && chatIdOrReplyTo !== null && chatIdOrReplyTo.chatId) {
        // replyTo-based call
        return { stateChanges: {}, effects: [sendEffect(chatIdOrReplyTo, text, { parse_mode: "HTML" })] }
    }
    // Legacy call: reply(chatId, text, threadId)
...
function handleTaskCreate(event, core, description) {
    const { chatId } = event
    if (!description) {
        return reply(event.replyTo, "Usage: <code>/task &lt;description&gt;</code>")
    }

    // Use the topic's session in command center mode, focused in DMs.
    // effectiveSessionId is resolved at the top of the handler.
    const access = loadAccess()
    const isCC = String(chatId) === String(access.commandCenterChatId ?? "")
    let focusedId = null
    if (isCC && event.threadId) {
        ...
        if (mappedSession && core.chatSessions?.[mappedSession]) {
            focusedId = mappedSession
        }
    }
    if (!focusedId) {
        focusedId = core.chatState?.focusedSessionId
    }
    if (!focusedId) {
        return reply(event.replyTo, "No focused session. Use /new to create one first.")
    }
    ...
    if (existingTaskId) {
        ...
        if (existing) {
            ...
            return reply(event.replyTo, lines.join("\n"))
        }
        dbg(
            "CHAT-USER",
...
function handleTaskStatus(event, core, taskId) {
    ...
    if (!task) {
        return reply(event.replyTo, `Task <code>${esc(taskId)}</code> not found.`)
    }
...
function handleTaskView(event, core, taskId) {
    ...
    return reply(event.replyTo, text)
}
...
function handleTaskCancel(event, core, taskId) {
    ...
    if (!result.ok) {
        return reply(event.replyTo, result.reason)
    }
...
function handleScheduleCreate(event, core, description) {
    const { chatId } = event
    if (!description) {
        return reply(event.replyTo, "Usage: <code>/schedule &lt;description&gt;</code>")
    }
    // Resolve session from topic in command center mode
    let focusedId = null
    const access = loadAccess()
    const isCC = String(chatId) === String(access.commandCenterChatId ?? "")
    if (isCC && event.threadId) {
...
function handleScheduleStatus(event, core, scheduleTaskId) {
    ...
    if (!found) {
        return reply(event.replyTo, `Unknown scheduled task: <code>${esc(scheduleTaskId)}</code>`)
    }
    const t = found.task
...
function handleScheduleEdit(event, core, scheduleTaskId, args) {
    ...
    if (task.state === "cancelled" || task.state === "completed") {
        return reply(event.replyTo, `Cannot edit — task is <code>${esc(task.state)}</code>.`)
    }
    ...
    if (naturalMatch) {
        ...
        for (const part of parts) {
            ...
            if (eqIdx < 1) {
                return reply(event.replyTo, `Invalid argument: <code>${esc(part)}</code>. Use <code>key=value</code> format or <code>every Nh</code>.`)
            }
            ...
            if (key === "freq") {
                ...
                return reply(event.replyTo, `Unknown rule key: <code>${esc(key)}</code>`)
            }
        }
    }

    if (Object.keys(patch).length === 0) {
        return reply(event.replyTo, `No changes parsed from: <code>${esc(args)}</code>`)
    }

    const newRule = { ...oldRule, ...patch }
    const check = validateRule(newRule)
    if (!check.ok) {
        return reply(event.replyTo, `Invalid rule after edit: ${esc(check.error)}`)
    }
