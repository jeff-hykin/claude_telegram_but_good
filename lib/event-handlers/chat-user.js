...
const { generateTaskId, randomHex } = await versionedImport("../pure/ids.js", import.meta)
...
export default async function handle(event, core) {
    ...
    if (trimmed.startsWith("/")) {
        ...
        if (cmdMatch) {
            ...
            const cmdAction = await dispatchHotCommand(cmdName, event, core)
...
function handleTaskCancel(event, core, taskId) {
    ...
    dbg("CHAT-USER", `cancelling long task ${taskId}`)
    ...
    const effects = [
        ...
        // Delete the on-disk definition backup — mirrors the cleanup in
        // critic-verdict.js's certified branch. Idempotent.
        {
            type: "delete_file",
            path: paths.longTaskDefinitionBackupFile(taskId),
        },
    ...
    //
    // Also clear the session's longTaskId pointer and the nudge action —
    // the Stop handler won't take the taskCheck branch for a session
    // whose longTaskId is undefined, so the session is free to host
    // another task later without stale watchdog state.
    const sessionStateChanges = workerSessionId
        ? {
            chatSessions: {
                [workerSessionId]: {
