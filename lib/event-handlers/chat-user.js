...
export default async function handle(event, core) {
    ...
    if (/^\/task_status(?:@\w+)?$/i.test(trimmed)) {
        ...
        return wrap(reply(rt, "No active task found for this topic. Use \`/task_status_<id>\` to specify one."))
    }
    ...
    if (isCommandCenter && event.threadId) {
        if (ccTopicSessionId && core.chatSessions?.[ccTopicSessionId]) {
            ...
            dbg("CHAT-USER", `stale threadMap entry: thread ${event.threadId} → ${ccTopicSessionId} (session gone)`)
            return wrap({
                stateChanges: {},
                effects: [sendEffect(event.replyTo, `Session \`${esc(ccTopicSessionId)}\` has disconnected. Use /refresh to respawn it.`, { parse_mode: "Markdown" })],
            })
        }
    }
    ...
    if (!session?._conn) {
        return wrap(reply(event.replyTo,
            `Session \`${esc(targetSessionId)}\` is disconnected — the Claude process is gone. Start a new one with /new.`,
        ))
    }
...
function enqueueWaitingMessage(event, core, trimmed) {
    ...
    return {
        ...
        effects: [sendEffect(rt, userText, { parse_mode: "Markdown" })],
    }
}
...
function handleAttachment(event, core) {
    ...
    if (!targetId) {
        return reply(attachRt, "No focused session. Use /list_sessions to see available sessions or /new to create one.")
    }
    const session = core.chatSessions?.[targetId]
    if (!session?._conn) {
        return reply(attachRt, `Session \`${esc(targetId)}\` is disconnected — the Claude process is gone. Start a new one with /new.`)
    }
...
function handleShellMessage(event, core, body) {
    ...
    if (parsed.kind === "empty") {
        return reply(replyTo, `_cwd: ${esc(currentCwd)}_`)
    }
    ...
    if (parsed.kind === "cd-home" || parsed.kind === "cd") {
        ...
        try {
            ...
            return reply(replyTo, `*cd:* ${esc(target)}: ${esc(msg)}`)
        }
        if (!stat.isDirectory) {
            return reply(replyTo, `*cd:* ${esc(target)}: not a directory`)
        }
        return {
            stateChanges: {
                chatState: {
            ...
            effects: [sendEffect(replyTo, `_cwd: ${esc(target)}_`, { parse_mode: "Markdown" })],
        }
    }
...
function handleTaskCreate(event, core, description) {
    ...
    if (existingTaskId) {
        ...
        if (existing) {
            ...
            const lines = [
                `Session \`${esc(focusedId)}\` already has an active task ` +
                `\`${esc(existingTaskId)}\` (state: \`${esc(existing.state ?? "?")}\`).`,
                ``,
                `Edit or cancel it first:`,
                ``,
                taskCommandLinks(existingTaskId),
            ]
            return reply(rt, lines.join("\n"))
        }
        dbg(
            "CHAT-USER",
    ...
    return {
        ...
        effects: [
            ...
            ...activation.effects,
            sendEffect(rt, `/task_cancel_${taskId}`, { parse_mode: "Markdown" }),
            {
                type: "cold_append",
                stream: "long-tasks",
                entry: {
...
function handleTaskStatus(event, core, taskId) {
    ...
    const lines = [
        `*Task* \`${esc(taskId)}\``,
        `*Title:* ${esc(task.title ?? "(untitled)")}`,
        `*State:* \`${esc(task.state ?? "unknown")}\``,
        `*Worker:* \`${esc(task.workerSessionId ?? "?")}\``,
        `*Created:* ${esc(formatAge(task.createdAt))}`,
        `*Definition:* ${esc(definitionStatus)}`,
        `*Critic calls:* ${esc(critStr)}`,
        `*Nudges:* ${esc(nudgeStr)}`,
        ``,
...
function handleTaskUpdate(event, core, taskId, body) {
    ...
    if (!newDefinition) {
        ...
        if (!task.definition) {
            return reply(rt, `Task \`${esc(taskId)}\` has no definition yet. Reply with \`/task_update_${esc(taskId)} <new text>\` to set one.`)
        }
        const def = task.definition.slice(0, 3000)
        const truncated = task.definition.length > def.length ? "\n\n[truncated]" : ""
        const text =
            `*Current definition — *\`${esc(taskId)}\`\n` +
            `\`\`\`\n${esc(def)}${esc(truncated)}\n\`\`\`\n` +
            `Reply with \`/task_update_${esc(taskId)} <new text>\` to replace.`
        return reply(rt, text)
    }
...
function handleScheduleStatus(event, core, scheduleTaskId) {
    ...
    const runsDisplay = maxRuns != null
        ...
        : String(totalRuns)
    const lines = [
        `*Scheduled task \`${esc(scheduleTaskId)}\`*`,
        `State: \`${esc(t.state ?? "?")}\``,
        `Next fire: \`${esc(t.tracking?.nextFireAt ?? "(none)")}\``,
        `Last run: \`${esc(t.tracking?.lastRunAt ?? "(never)")}\` — ${esc(t.tracking?.lastRunStatus ?? "(none)")}`,
        `Runs: \`${esc(runsDisplay)}\`${maxRuns != null ? (maxRuns === 1 ? " (once)" : ` (${maxRuns}x)`) : " (recurring)"}`,
        `Skip next: \`${esc(String(t.tracking?.skipNext ?? false))}\``,
        ``,
...
function handleScheduleView(event, core, scheduleTaskId) {
    ...
    const history = (t.tracking?.runHistory ?? []).slice(-5).map((r) =>
        `- ${esc(r.at ?? "?")}: \`${esc(r.status ?? "?")}\` ${esc((r.summary ?? "").slice(0, 100))}`,
    )
    const lines = [
        `*Scheduled task \`${esc(scheduleTaskId)}\`*`,
        `Title: ${esc(t.title ?? "")}`,
        ``,
        `*Rule*`,
        `\`\`\`\n${esc(JSON.stringify(t.rule ?? {}, null, 2))}\n\`\`\``,
        ``,
        `*Definition of done*`,
        `\`\`\`\n${esc((t.definitionOfDone ?? "(none)").slice(0, 2000))}\n\`\`\``,
        ``,
        `*Recent runs*`,
        ...
        effects: [
            {
                type: "send_text_to_user",
                chatId: event.chatId,
                text: `Scheduled task \`${esc(scheduleTaskId)}\`: skipNext=${next ? "*true*" : "false"}`,
                options: { parse_mode: "Markdown", ...(event.threadId != null ? { message_thread_id: Number(event.threadId) } : {}) },
            },
        ],
    }
...
function handleScheduleEdit(event, core, scheduleTaskId, args) {
    ...
    if (!args) {
        const lines = [
            `*Edit scheduled task \`${esc(scheduleTaskId)}\`*`,
            ``,
            `Current rule:`,
            `\`\`\`\n${esc(JSON.stringify(oldRule, null, 2))}\n\`\`\``,
            ``,
            `Usage:`,
            `\`/schedule_edit_${esc(scheduleTaskId)} every 4h\``,
            `\`/schedule_edit_${esc(scheduleTaskId)} interval=6\``,
            `\`/schedule_edit_${esc(scheduleTaskId)} freq=DAILY byhour=9\``,
            ``,
    ...
    return {
        ...
        effects: [
            ...
            },
            {
                type: "send_text_to_user",
                chatId: event.chatId,
                text: `Updated rule for \`${esc(scheduleTaskId)}\`:\n\`\`\`\n${esc(JSON.stringify(newRule, null, 2))}\n\`\`\`\n\nTimer rearmed.`,
                options: { parse_mode: "Markdown", ...(event.threadId != null ? { message_thread_id: Number(event.threadId) } : {}) },
            },
        ],
    }
