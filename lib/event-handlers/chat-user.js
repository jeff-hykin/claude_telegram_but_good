...
const { makeReplyTo, replyToFromEvent } = await versionedImport("../pure/reply-to.js", import.meta)
            ...
            effects: [
                {
                    ...
                    replyTo: event._replyTo ?? replyToFromEvent(event, "chat-user:unknown-cmd"),
    ...
    try {
        ...
        return {
            ...
            effects: [
                {
                    type: "send_text_to_user",
                    replyTo: event._replyTo ?? replyToFromEvent(event, "chat-user:cmd-error"),
...
export default async function handle(event, core) {
    ...
    // Build the canonical replyTo for this inbound message. Every
    // outbound effect produced by this handler (and by commands it
    // dispatches) uses this object for routing. The event is stashed
    // on the event object so commands can access it without threading
    // it through every function signature.
    const replyTo = replyToFromEvent(event, "chat-user:inbound")
    event._replyTo = replyTo
...
function enqueueWaitingMessage(event, core, trimmed) {
    ...
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:enqueue")
...
function handleAttachment(event, core) {
    ...
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:attachment")
    ...
    if (!session?._conn) {
        return reply(
            replyTo,
            `Focused session <code>${esc(focusedId)}</code> is disconnected — the Claude process is gone. Start a new one with /new.`,
        )
    }
...
function handleClose(event, core, targetId) {
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:close")
    const target = core.chatSessions?.[targetId]
    if (!target) {
        return reply(replyTo, `Session <code>${esc(targetId)}</code> not found.`)
    }
...
function handleSwitch(event, core, targetId) {
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:switch")
...
function handleTaskCreate(event, core, description) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:task-create")
    if (!description) {
        return reply(replyTo, "Usage: <code>/task &lt;description&gt;</code>")
    }

    const focusedId = core.chatState?.focusedSessionId
    if (!focusedId) {
        return reply(replyTo, "No focused session. Use /new to create one first.")
    }
...
function handleTaskStatus(event, core, taskId) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:task-status")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(replyTo, `Task <code>${esc(taskId)}</code> not found.`)
    }
...
function handleTaskView(event, core, taskId) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:task-view")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(replyTo, `Task <code>${esc(taskId)}</code> not found.`)
    }
    ...
    return reply(replyTo, text)
}

function handleTaskUpdate(event, core, taskId, body) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:task-update")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(replyTo, `Task <code>${esc(taskId)}</code> not found.`)
    }
...
function handleTaskCancel(event, core, taskId) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:task-cancel")
    const result = buildCancelAction(core, chatId, taskId)
    if (!result.ok) {
        return reply(replyTo, result.reason)
    }
...
function handleTaskResume(event, core, taskId) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:task-resume")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(replyTo, `Task <code>${esc(taskId)}</code> not found.`)
    }
...
function handleScheduleCreate(event, core, description) {
    const { chatId } = event
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:schedule-create")
    if (!description) {
        return reply(replyTo, "Usage: <code>/schedule &lt;description&gt;</code>")
    }
    const focusedId = core.chatState?.focusedSessionId
    if (!focusedId) {
        return reply(replyTo, "No focused session. Use /new to create one first.")
...
function handleScheduleStatus(event, core, scheduleTaskId) {
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:schedule-status")
    const found = findScheduledTask(core.specialData, scheduleTaskId)
    if (!found) {
        return reply(replyTo, `Unknown scheduled task: <code>${esc(scheduleTaskId)}</code>`)
    }
    const t = found.task
    const lines = [
...
function handleScheduleView(event, core, scheduleTaskId) {
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:schedule-view")
    const found = findScheduledTask(core.specialData, scheduleTaskId)
    if (!found) {
        return reply(replyTo, `Unknown scheduled task: <code>${esc(scheduleTaskId)}</code>`)
    }
...
function handleSchedulePause(event, core, scheduleTaskId) {
    const replyTo = event._replyTo ?? replyToFromEvent(event, "chat-user:schedule-pause")
    const found = findScheduledTask(core.specialData, scheduleTaskId)
    if (!found) {
        return reply(replyTo, `Unknown scheduled task: <code>${esc(scheduleTaskId)}</code>`)
    }
