...
const { makeReplyTo, sendEffect, replyToForSession } = await versionedImport("../pure/reply-to.js", import.meta)
...
async function dispatchHotCommand(cmdName, event, core) {
    ...
    if (typeof handler !== "function") {
        ...
        const rt = event.replyTo ?? makeReplyTo(event, "dispatch/unknown-cmd")
        return {
...
export default async function handle(event, core) {
    ...
    event.replyTo = makeReplyTo(event, "chat-user/handle")
...
function enqueueWaitingMessage(event, core, trimmed) {
    ...
    const rt = event.replyTo ?? makeReplyTo(event, "chat-user/enqueue")
    return {
        stateChanges: {
 ...
 * Accepts either a replyTo object or (chatId, text, threadId) for
 * backward compat during migration.
 */
function reply(chatIdOrReplyTo, text, threadId) {
    if (typeof chatIdOrReplyTo === "object" && chatIdOrReplyTo.chatId) {
        // New replyTo path
        return {
            stateChanges: {},
            effects: [sendEffect(chatIdOrReplyTo, text, { parse_mode: "HTML" })],
        }
    }
    // Legacy path (chatId string)
    return {
        stateChanges: {},
        effects: [sendEffect(
            { chatId: chatIdOrReplyTo, threadId: threadId != null ? Number(threadId) : null, setBy: "chat-user/legacy" },
            text,
            { parse_mode: "HTML" },
        )],
    }
...
function handleTaskCreate(event, core, description) {
    const { chatId } = event
    if (!description) {
        const rt0 = event.replyTo ?? makeReplyTo(event, "task-create")
        return reply(rt0, "Usage: <code>/task &lt;description&gt;</code>")
    }

    // In CC mode, use the topic's session. In DM, use focused.
    const rt = event.replyTo ?? makeReplyTo(event, "task-create")
    let focusedId = null
    if (event.threadId) {
        const cc = core.chatState?.commandCenter ?? {}
...
function handleScheduleCreate(event, core, description) {
    const { chatId } = event
    const rt = event.replyTo ?? makeReplyTo(event, "schedule-create")
    if (!description) {
