...
const { loadAccess, classifyGroup, isBotMentioned } = await versionedImport("../access.js", import.meta)
...
const { buildIntervalHookOffAction, buildIntervalHookViewAction } = await versionedImport("../interval-hook-actions.js", import.meta)
const { validateRule } = await versionedImport("../scheduler/index.js", import.meta)
const { replyToFromEvent, sendEffect, replyToForSession } = await versionedImport("../pure/reply-to.js", import.meta)
const { topicShellKey, parseShellMessage, resolveCdTarget } = await versionedImport("../pure/shell-cwd.js", import.meta)
const { loginTopicKey, looksLikeLoginCode } = await versionedImport("../pure/login.js", import.meta)
const { matchHooks, formatHintBlock, removeHook } = await versionedImport("../memory-hooks.js", import.meta)
...
async function dispatchHotCommand(cmdName, event, core) {
    ...
    if (typeof handler !== "function") {
        ...
        const rt = event.replyTo ?? replyToFromEvent(event, "dispatch/unknown-cmd")
        return {
            effects: [sendEffect(rt, `Unknown command: /${cmdName}. Use /help to see available commands.`)],
        }
    }
...
/**
 * Build channel event content, prepending reply-to context as XML when
 * present and appending any memory hints this message tripped (see
 * lib/memory-hooks.js). Hints go last so they read as a footnote to the
 * user's words rather than as part of them.
 */
function buildChannelContent(messageText, event, hooks = []) {
    let content = messageText
    if (event.replyToText) {
        content = `<reply_to message_id="${event.replyToMessageId}">\n${event.replyToText}\n</reply_to>\n${messageText}`
    }
    const hints = formatHintBlock(hooks)
...
export default async function handle(event, core) {
    ...
    // chatState every return path must carry regardless of which branch
    // builds the Action (currently only disarming a pending login).
    let carriedChatState = null
    function wrap(action) {
        ...
        return {
            ...action,
            effects: [...(action?.effects ?? []), ...recordEffects],
            stateChanges: {
                ...prev,
                ...(carriedChatState ? { chatState: { ...carriedChatState, ...(prev.chatState ?? {}) } } : {}),
                specialData: {
                    ...prevSpecial,
    ...
    // 1a. Group bucket. Groups CBG was not explicitly told about are
    //     GroupChats: ordinary human conversations the bot happens to
    //     sit in. Each one gets its own session that reads the whole
    //     conversation but is held silent at the daemon level (see
    //     lib/listen-mode.js) until someone addresses the bot. Slash
    //     commands still dispatch normally for allowlisted senders so
    //     the group's session can be managed from inside the group.
    const isGroup = event.chatType === "group" || event.chatType === "supergroup"
    const isGroupChat = isGroup && classifyGroup(chatId, access) === "groupChat"
    // First sighting of a group materializes it in the GroupChats list
    // so the bucket it landed in is visible and can be promoted.
    const recordEffects = isGroupChat && !access.groupChats.includes(String(chatId))
        ? [{ type: "record_group_chat", chatId: String(chatId) }]
        : []
    if (isGroupChat && !(isAllowed && trimmed.startsWith("/"))) {
        return wrap(handleGroupChatMessage(event, core, trimmed, isBotMentioned(event, access)))
    }
    ...
    // 1.5 Stamp replyTo on the event so all downstream code knows
    // where to send replies. For CC topics, also resolve the target
    // session from topicMap — CC messages should route to the topic's
    // session, not the globally focused one.
    event.replyTo = replyToFromEvent(event, "chat-user/handle")

    let ccTopicSessionId = null
    if (isCommandCenter && event.threadId) {
        const cc = core.chatState?.commandCenter ?? {}
        ccTopicSessionId = cc.threadMap?.[String(event.threadId)] ?? null
    }

    // 1.6 Attachment handling. Runs BEFORE the command-regex dispatches
    // so attachments always go to the target session instead of being
    // parsed as slash commands (their synthesized text starts with "(").
    if (event.attachment) {
        return wrap(handleAttachment(event, core))
    }

    // 1.7 A /login in this topic is waiting on the code from the browser.
    // The very next message decides: a code gets typed into the session,
    // anything else disarms and carries on as an ordinary message.
    const loginKey = loginTopicKey(chatId, event.threadId)
    if (core.chatState?.pendingLogin?.[loginKey] && !isCommand) {
        carriedChatState = { pendingLogin: { [loginKey]: undefined } }
        if (looksLikeLoginCode(trimmed)) {
            return wrap(handleLoginCode(event, core, trimmed, core.chatState.pendingLogin[loginKey]))
        }
        dbg("CHAT-USER", `pending login in ${loginKey} disarmed — message isn't a code`)
    }
    ...
    // /connect_<id> — bind a session to the current topic.
    const connectMatch = /^\/connect_([a-zA-Z0-9_]+)/i.exec(trimmed)
    if (connectMatch) {
        return wrap(handleConnect(event, core, connectMatch[1]))
    }

    // 3. Task dynamic commands.
    // Bare /task_status (no ID) — auto-resolve from topic's active task
    if (/^\/task_status(?:@\w+)?$/i.test(trimmed)) {
        const resolved = resolveTaskFromTopic(event, core)
        if (resolved) {
            return wrap(handleTaskStatus(event, core, resolved))
        }
        const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/task-status-auto")
        return wrap(replyPlain(rt, "No active task found for this topic. Use /task_status_<id> to specify one."))
    }
    ...
    const taskNewMatch = /^\/task\s+([\s\S]+)/i.exec(trimmed)
    if (taskNewMatch) {
    ...
    const scheduleEditMatch = /^\/schedule_edit_(\w+)(?:\s+(.+))?/i.exec(trimmed)
    if (scheduleEditMatch) {
        return wrap(handleScheduleEdit(event, core, scheduleEditMatch[1], (scheduleEditMatch[2] ?? "").trim()))
    }
    ...
    // 3c. Interval-hook dynamic commands. Specific /interval_hook_*_<id>
    // matchers only — creation is agent-driven via the MCP tool.
    const intervalHookOffMatch = /^\/interval_hook_off_(\w+)/i.exec(trimmed)
    if (intervalHookOffMatch) {
        return wrap(buildIntervalHookOffAction(core, event.chatId, intervalHookOffMatch[1]))
    }
    const intervalHookViewMatch = /^\/interval_hook_view_(\w+)/i.exec(trimmed)
    if (intervalHookViewMatch) {
        return wrap(buildIntervalHookViewAction(core, event.chatId, intervalHookViewMatch[1]))
    }

    // 3e. /memory_hook_remove_<id> — the tappable link /memory_hooks prints.
    const memoryHookRemoveMatch = /^\/memory_hook_remove_(\w+)/i.exec(trimmed)
    if (memoryHookRemoveMatch) {
        const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/memory-hook-remove")
        const result = removeHook(memoryHookRemoveMatch[1])
        return wrap(replyPlain(
            rt,
            result.error
                ? result.error
                : `Removed memory hook ${result.hook.pattern}. ${result.total} left.`,
        ))
    }

    // 3d. `//foo` is shorthand for `/raw /foo` — a slash command meant for
    // the session's own TUI rather than for the bot. Without it, anything
    // Claude Code and cbg both define (/model, /compact) is unreachable.
    if (/^\/\/\S/.test(trimmed)) {
        return wrap(await dispatchHotCommand("raw", { ...event, text: `/raw ${trimmed.slice(1)}` }, core))
    }
    ...
    // 4.5 `#`/`!`-prefix shell commands. Per-topic pwd, hardcoded `# cd`
    // updates state without forking, anything else spawns `zsh -c`
    // via the shell_command_spawn effect. Same gating as plain text.
    if (trimmed.startsWith("#") || trimmed.startsWith("!")) {
        if (!isAllowed) {
            dbg("CHAT-USER", `DROPPED: shell from non-allowed user ${senderId}`)
            return wrap({ stateChanges: {}, effects: [] })
        }
        return wrap(handleShellMessage(event, core, trimmed.slice(1)))
    }
    ...
    // ccTopicSessionId was resolved at the top of handle().
    if (isCommandCenter && event.threadId) {
        if (ccTopicSessionId) {
            dbg("CHAT-USER", `command center topic routing: thread ${event.threadId} → session ${ccTopicSessionId}`)
            targetSessionId = ccTopicSessionId
        } else {
            // Orphan topic — no session bound
            return wrap({
                stateChanges: {},
                effects: [sendEffect(event.replyTo, "No session is attached to this topic. Use /refresh to spawn one.", { parse_mode: "Markdown" })],
            })
        }
    }
    ...
    if (!session?._conn) {
        dbg("CHAT-USER", `session ${targetSessionId} disconnected — queueing message for reconnect`)
        return wrap(enqueueWaitingMessage(event, core, trimmed, targetSessionId))
    }
    ...
    // Memory hooks tripped by what the user just said. Recording them as
    // fired starts the turn's exclusion set, so a hint delivered here
    // won't also bounce the agent's reply back at it moments later.
    const userHooks = matchHooks(trimmed, "user")
    const firedIds = userHooks.map((hook) => hook.id).filter(Boolean)
        ...
        stateChanges: {
            chatSessions: {
                [targetSessionId]: {
                    ...
                    ...(firedIds.length > 0 ? { memoryHooksFired: firedIds } : {}),
                    ...activation.patch,
                },
            },
        },
        effects: [
            {
                ...
                content: buildChannelContent(trimmed, event, userHooks),
...
/**
 * A message in a GroupChat — a group nobody explicitly configured CBG
 * for. The group gets a session of its own that reads the entire
 * conversation; lib/listen-mode.js is what stops it from posting there
 * unless this particular message addresses the bot.
 *
 * Messages that arrive before the session finishes starting up are
 * dropped rather than queued: a group's backlog is ambient chatter, not
 * instructions waiting to be carried out.
 */
function handleGroupChatMessage(event, core, trimmed, mentionsBot) {
    const { chatId, userId, messageId } = event
    const ensured = ensureGroupSession(core, chatId, event._ctx?.chat?.title ?? null, event.ts)
    if (!ensured.live) {
        // A message that addresses the bot is a question someone expects
        // an answer to, so it waits for the session to finish starting.
        // Ambient chatter is dropped — a group's backlog is conversation,
        // not instructions.
        if (!mentionsBot || !ensured.sessionId) {
            dbg("CHAT-USER", `GroupChat ${chatId}: no live session yet — dropping message`)
            return { stateChanges: ensured.stateChanges, effects: ensured.effects }
        }
        // Queued by hand rather than through enqueueWaitingMessage, which
        // posts a "message queued" notice into the chat — the bot must not
        // announce itself in a group it is only listening to.
        dbg("CHAT-USER", `GroupChat ${chatId}: queueing addressed message for starting session ${ensured.sessionId}`)
        const queue = core.chatState?.messageQueue ?? []
        const entry = {
            content: buildChannelContent(trimmed, event),
            meta: { message_id: String(messageId), chat_id: chatId },
            threadId: event.threadId ?? null,
            queuedAt: event.ts,
            targetSessionId: ensured.sessionId,
        }
        const ensuredSessions = ensured.stateChanges.chatSessions ?? {}
        return {
            stateChanges: {
                chatState: {
                    ...(ensured.stateChanges.chatState ?? {}),
                    messageQueue: queue.length >= MESSAGE_QUEUE_CAP
                        ? [...queue.slice(1), entry]
                        : [...queue, entry],
                },
                chatSessions: {
                    ...ensuredSessions,
                    // Unlock the turn the session will wake up into —
                    // otherwise it starts life muzzled and cannot answer
                    // the very message that summoned it.
                    [ensured.sessionId]: {
                        ...(ensuredSessions[ensured.sessionId] ?? {}),
                        listenUnlockedAt: event.ts,
                    },
                },
            },
    ...
    // Only default the flag on — an explicit `/listen off` must stick.
    if (session.listenMode === undefined) {
        sessionPatch.listenMode = true
        sessionPatch.listenChatId = String(chatId)
    }

    const userHooks = matchHooks(trimmed, "user")
    const firedIds = userHooks.map((hook) => hook.id).filter(Boolean)
    if (firedIds.length > 0) {
        sessionPatch.memoryHooksFired = firedIds
    }
    ...
    if (!mentionsBot) {
        // Nobody is waiting on an answer, so skip the spinner (it would
        // post a visible "thinking" message into a group that expects
        // silence) and skip the waiting-state watchdog (it nudges the
        // agent to reply, which listen mode would then refuse).
        dbg("CHAT-USER", `GroupChat ${chatId}: listening via ${sessionId}`)
        return {
            stateChanges: { chatSessions: { [sessionId]: sessionPatch } },
            effects,
            noSpinner: true,
        }
    }

    // Addressed directly — unlock replies for this turn. claude-hook-stop
    // clears the unlock once the turn ends.
    sessionPatch.listenUnlockedAt = event.ts
    const activation = activateWaitingState(session, sessionId, event.ts, "askAgentToSendChatMessage")
    dbg("CHAT-USER", `GroupChat ${chatId}: bot addressed, unlocking ${sessionId} for this turn`)
    return {
        stateChanges: { chatSessions: { [sessionId]: { ...sessionPatch, ...activation.patch } } },
        effects: [...effects, ...activation.effects],
    }
}
...
function enqueueWaitingMessage(event, core, trimmed, targetSessionId = null) {
    ...
    let nextQueue
    let userText = targetSessionId
        ? `Session disconnected — message queued. Will deliver when it reconnects.`
        : "No session available; queued. Will deliver when a session registers."
    ...
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/enqueue")
    return {
        stateChanges: {
            chatState: { messageQueue: nextQueue },
        },
        effects: [sendEffect(rt, userText, {})],
    }
}
...
function handleAttachment(event, core) {
    ...
    const attachRt = event.replyTo ?? replyToFromEvent(event, "chat-user/attachment")
    if (!targetId) {
        return replyPlain(attachRt, "No session available. Use /new to create one.")
    }

    // Pass 1: always download first — don't need an active session for that.
    ...
    // Session may have disconnected between pass 1 and pass 2 — queue.
    const session = core.chatSessions?.[targetId]
    if (!session?._conn) {
        dbg("CHAT-USER", `session ${targetId} disconnected — queueing attachment for reconnect`)
        return enqueueWaitingMessage(event, core, content, targetId)
    }
        ...
        effects: [
            {
                type: "deliver_channel_event",
                sessionId: focusedId,
                content: buildChannelContent(content, event),
                meta,
            },
            ...activation.effects,
            {
...
// Typing the code char-by-char is slow, and exchanging it is a network
// round-trip, so the confirmation panel is this far out at best.
const LOGIN_CONFIRM_DELAY_MS = 2000

/**
 * Type the OAuth code the user pasted back into the session that asked for
 * it, then show what the TUI did with it. Armed by commands/login.js; the
 * caller has already disarmed it so this only ever runs once per /login.
 */
function handleLoginCode(event, core, code, pendingLogin) {
    const { sessionId } = pendingLogin
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/login-code")
    if (!core.chatSessions?.[sessionId]?.dtachSocket) {
        return replyPlain(rt, `Session "${sessionId}" is gone — run /login again.`)
    }
    dbg("CHAT-USER", `typing login code into session ${sessionId}`)
    return {
        stateChanges: {},
        effects: [
            // This submits the code itself. What it does NOT do is get the
            // prompt back: /login parks on "Press Enter to continue…" and
            // stays there, so hand off to "/login __confirm", which watches
            // for that panel and presses Return once it's up.
            { type: "send_raw_input_to_claude", sessionId, text: code },
            {
                type: "set_timer",
                delayMs: LOGIN_CONFIRM_DELAY_MS,
                event: {
                    ...
                    userId: event.userId,
                    username: event.username ?? null,
                    messageId: `login_confirm_${Date.now()}`,
                    text: "/login __confirm",
                    ts: Date.now(),
                },
            },
        ],
...
function handleClose(event, core, targetId) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/close")
    const target = core.chatSessions?.[targetId]
    if (!target) {
        return reply(rt, `Session \`${esc(targetId)}\` not found.`)
    }
    const title = target.title ? ` (${esc(target.title)})` : ""
    const closeOpts = { parse_mode: "Markdown" }
    ...
    return {
        ...
        effects: [
            ...
            { type: "send_text_to_claude", sessionId: targetId, text: "/exit" },
            {
                type: "send_text_to_user",
                chatId,
                text: `Closing session \`${esc(targetId)}\`${title}… sent \`/exit\`, will SIGTERM in ${Math.round(CLOSE_GRACE_MS / 1000)}s if still alive.`,
                options: closeOpts,
            },
...
function handleConnect(event, core, targetId) {
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/connect")
    const threadId = event.threadId ? String(event.threadId) : null

    if (!threadId) {
        return reply(rt, "Use /connect from a topic thread, not the General chat.")
    }
    ...
    if (!session._conn) {
        return reply(rt, `Session \`${esc(targetId)}\` is disconnected.`)
    }

    const cc = core.chatState?.commandCenter ?? {}
    const topicName = cc.topicNames?.[threadId] ?? `topic_${threadId}`

    // Unbind the old session from this topic (if any)
    const oldSessionId = cc.threadMap?.[threadId] ?? null
    ...
    // Remove old binding
    if (oldSessionId && topicMap[oldSessionId] === threadId) {
        delete topicMap[oldSessionId]
    }

    // Add new binding
    topicMap[targetId] = threadId
    threadMap[threadId] = targetId

    const title = session.title ?? targetId
    return {
        stateChanges: {
            chatState: {
        ...
        effects: [sendEffect(rt,
            `Connected \`${esc(targetId)}\` (${esc(title)}) to this topic.${oldSessionId ? `\nUnbound previous session \`${esc(oldSessionId)}\`.` : ""}`,
            { parse_mode: "Markdown" },
        )],
    }
}
...
function handleSwitch(event, core, targetId) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/switch")
    if (!core.chatSessions?.[targetId]) {
        return reply(rt, `Session \`${esc(targetId)}\` not found.`)
    }
    const target = core.chatSessions[targetId]
    const parts = [`Switched to session \`${esc(targetId)}\``]
    if (target.title) {
        ...
        effects: [
            {
                ...
                chatId,
                text: parts.join(" — "),
                options: { parse_mode: "Markdown", ...(event.threadId != null ? { message_thread_id: Number(event.threadId) } : {}) },
            },
        ],
    }
}
...
/**
 * Build a reply Action from a replyTo object.
 */
...
function replyPlain(replyTo, text) {
    return {
        stateChanges: {},
...
// ── `#`/`!`-prefix shell handler ──────────────────────────────────────
//
// Per-topic pwd state lives at chatState.shellCwd[<key>]. `# cd <path>`
// (or `! cd …`) is hardcoded here so we don't fork — we resolve the
// new path, validate it exists, and emit a state patch. Everything
// else is dispatched as `shell_command_spawn` to lib/effects/shell-command.js.

function handleShellMessage(event, core, body) {
    const replyTo = event.replyTo ?? replyToFromEvent(event, "chat-user/shell")
    const key = topicShellKey(event.chatId, event.threadId)
    const homeDir = Deno.env.get("HOME") ?? "/"
    const currentCwd = core.chatState?.shellCwd?.[key] ?? homeDir

    const parsed = parseShellMessage(body)
    if (parsed.kind === "empty") {
        return reply(replyTo, `_cwd:_ \`${currentCwd}\``)
    }

    if (parsed.kind === "cd-home" || parsed.kind === "cd") {
        const target = parsed.kind === "cd-home" ? homeDir : resolveCdTarget(currentCwd, parsed.target, homeDir)
        let stat
        try {
            stat = Deno.statSync(target)
        } catch (e) {
            const msg = e instanceof Error ? e.message : String(e)
            return reply(replyTo, `*cd:* \`${target}\`: ${esc(msg)}`)
        }
        if (!stat.isDirectory) {
            return reply(replyTo, `*cd:* \`${target}\`: not a directory`)
        }
        return {
            stateChanges: {
                chatState: {
            ...
            effects: [sendEffect(replyTo, `_cwd:_ \`${target}\``, { parse_mode: "Markdown" })],
        }
    }
...
/**
 * Given a message event from a command center topic, resolve the active
 * task for that topic. Looks up threadId → session, then finds the
 * in_progress task whose workerSessionId matches.
 */
function resolveTaskFromTopic(event, core) {
    const cc = core.chatState?.commandCenter ?? {}
    const threadId = event.threadId
    if (!threadId) { return null }

    // Find the session mapped to this thread
    const sessionId = cc.threadMap?.[String(threadId)]
    if (!sessionId) { return null }

    // Search all tasks for one whose workerSessionId matches
    const allTasks = core.specialData?.longTaskByChatId ?? {}
    for (const taskMap of Object.values(allTasks)) {
        if (!taskMap || typeof taskMap !== "object") { continue }
        for (const task of Object.values(taskMap)) {
            if (task?.workerSessionId === sessionId && task?.state === "in_progress") {
                return task.id
            }
        }
    }
...
function handleTaskCreate(event, core, description) {
    const { chatId } = event
    if (!description) {
        const rt0 = event.replyTo ?? replyToFromEvent(event, "task-create")
        return reply(rt0, "Usage: \`/task <description>\`")
    }

    // In CC mode, use the topic's session. In DM, use focused.
    const rt = event.replyTo ?? replyToFromEvent(event, "task-create")
    let focusedId = null
    if (event.threadId) {
        const cc = core.chatState?.commandCenter ?? {}
        focusedId = cc.threadMap?.[String(event.threadId)] ?? null
    }
    if (!focusedId) { focusedId = core.chatState?.focusedSessionId }
    if (!focusedId) {
        return reply(rt, "No focused session. Use /new to create one first.")
    }
    ...
    if (existingTaskId) {
        ...
        if (existing) {
            ...
            const lines = [
                `Session ${focusedId} already has an active task ` +
                `${existingTaskId} (state: ${existing.state ?? "?"}).`,
                ``,
                `Edit or cancel it first:`,
                ``,
                taskCommandLinks(existingTaskId),
            ]
            return replyPlain(rt, lines.join("\n"))
        }
        dbg(
            "CHAT-USER",
    ...
    const prompt = [
        ...
        ``,
        `When you use the \`reply\` tool, pass \`format: "markdown"\` for any text with`,
        `formatting — Telegram is now using legacy Markdown (\`*bold*\`, \`_italic_\`,`,
        "`` `inline code` ``, ``` ```code blocks``` ```, `[text](url)`). HTML tags do",
        `NOT render — \`<b>\` etc. show up literally.`,
        ``,
        `The definition of done controls what the critic will verify. If the critic`,
        `should run tests, say so. If it should NOT run tests (e.g. no test infra`,
        `yet), say so. If it should check specific files, review code, or verify`,
        `commands succeed — state it explicitly. The critic will do exactly what the`,
        `definition tells it to and nothing more.`,
        ``,
        `DO NOT include "delivered via Telegram" or any variant as a criterion.`,
        `CBG tracks message delivery out-of-band — it is not the worker's or`,
        `critic's job to prove Telegram delivery happened.`,
        ``,
        ...
        ``,
        `Whenever you ask the user a question, SET A TIMER FOR 30MIN (via the`,
        "`set_reminder` MCP tool). If the question is not answered within that",
        `time, take your best guess and keep going. Don't block the whole long`,
        `task just because the user got busy.`,
        ``,
        `<user_prompt>`,
        description,
        `</user_prompt>`,
    ...
    return {
        ...
        effects: [
            ...
            ...activation.effects,
            sendEffect(rt, `/task_cancel_${taskId}`, {}),
            {
                type: "cold_append",
                stream: "long-tasks",
                entry: {
...
function handleTaskStatus(event, core, taskId) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/task-status")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(rt, `Task \`${esc(taskId)}\` not found.`)
    }
...
function handleTaskView(event, core, taskId) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/task-view")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(rt, `Task \`${esc(taskId)}\` not found.`)
    }
    if (!task.definition) {
        return reply(rt, `Task \`${esc(taskId)}\` has no definition yet (still in \`defining\` state).`)
    }
    ...
    const text = `*Definition of done — *\`${esc(taskId)}\`\n\`\`\`\n${def}${truncated}\n\`\`\``
    return reply(rt, text)
}

function handleTaskUpdate(event, core, taskId, body) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/task-update")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(rt, `Task \`${esc(taskId)}\` not found.`)
    }
    ...
    if (!newDefinition) {
        ...
        if (!task.definition) {
            return replyPlain(rt, `Task ${taskId} has no definition yet. Reply with /task_update_${taskId} <new text> to set one.`)
        }
        const def = task.definition.slice(0, 3000)
        const truncated = task.definition.length > def.length ? "\n\n[truncated]" : ""
        const text =
            `Current definition — ${taskId}\n\n` +
            `${def}${truncated}\n\n` +
            `Reply with /task_update_${taskId} <new text> to replace.`
        return replyPlain(rt, text)
    }
    ...
    const replyText = resumedFromClarification
        ? `Definition updated for \`${esc(taskId)}\`. Resumed.`
        : `Definition updated for \`${esc(taskId)}\`.`
...
function handleTaskCancel(event, core, taskId) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/task-cancel")
    const result = buildCancelAction(core, chatId, taskId)
    if (!result.ok) {
        return replyPlain(rt, result.reason)
    }
...
function handleTaskResume(event, core, taskId) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "chat-user/task-resume")
    const task = findTask(core, chatId, taskId)
    if (!task) {
        return reply(rt, `Task \`${esc(taskId)}\` not found.`)
    }
    if (task.state !== "cancelled") {
        return reply(rt,
            `Task \`${esc(taskId)}\` is not cancelled (current state: ` +
            `\`${esc(task.state ?? "?")}\`). Nothing to resume.`)
    }

    const workerSessionId = task.workerSessionId
    if (!workerSessionId) {
        return reply(rt, `Task \`${esc(taskId)}\` has no worker session recorded. Cannot resume.`)
    }
    const workerSession = core.chatSessions?.[workerSessionId]
    if (!workerSession) {
        return reply(rt, `Worker session \`${esc(workerSessionId)}\` for this task is gone. Cannot resume.`)
    }
    if (workerSession.longTaskId && workerSession.longTaskId !== taskId) {
        const existing = workerSession.longTaskId
        return reply(rt,
            `Worker session \`${esc(workerSessionId)}\` is already running ` +
            `task \`${esc(existing)}\`. Cancel that one first.`)
    }
...
function handleScheduleCreate(event, core, description) {
    const { chatId } = event
    const rt = event.replyTo ?? replyToFromEvent(event, "schedule-create")
    if (!description) {
        return reply(rt, "Usage: \`/schedule <description>\`")
    }
    // In CC mode, use the topic's session. In DM, use focused.
    let focusedId = null
    if (event.threadId) {
    ...
    return {
        ...
        effects: [
            ...
            sendEffect(rt, `Drafting scheduled task \`${esc(scheduleTaskId)}\` — clarify with the agent, then it'll be locked.`, { parse_mode: "Markdown" }),
        ],
    }
}
...
function handleScheduleStatus(event, core, scheduleTaskId) {
    ...
    if (!found) {
        return reply(event.chatId, `Unknown scheduled task: \`${esc(scheduleTaskId)}\``, event.threadId)
    }
    const t = found.task
    ...
    const runsDisplay = maxRuns != null
        ...
        : String(totalRuns)
    const lines = [
        `Scheduled task ${scheduleTaskId}`,
        `State: ${t.state ?? "?"}`,
        `Next fire: ${t.tracking?.nextFireAt ?? "(none)"}`,
        `Last run: ${t.tracking?.lastRunAt ?? "(never)"} — ${t.tracking?.lastRunStatus ?? "(none)"}`,
        `Runs: ${runsDisplay}${maxRuns != null ? (maxRuns === 1 ? " (once)" : ` (${maxRuns}x)`) : " (recurring)"}`,
        `Skip next: ${t.tracking?.skipNext ?? false}`,
        ``,
        scheduleCommandLinks(scheduleTaskId),
    ]
    return replyPlain(replyToFromEvent(event, "schedule"), lines.join("\n"))
}
...
function handleScheduleView(event, core, scheduleTaskId) {
    ...
    const history = (t.tracking?.runHistory ?? []).slice(-5).map((r) =>
        `- ${r.at ?? "?"}: ${r.status ?? "?"} ${(r.summary ?? "").slice(0, 100)}`,
    )
    const lines = [
        `Scheduled task ${scheduleTaskId}`,
        `Title: ${t.title ?? ""}`,
        ...
        ``,
        `Definition of done:`,
        (t.definitionOfDone ?? "(none)").slice(0, 2000),
        ``,
        `Recent runs:`,
        ...
        effects: [
            {
                type: "send_text_to_user",
                chatId: event.chatId,
                text: `Scheduled task ${scheduleTaskId}: skipNext=${next}`,
...
/**
 * /schedule_edit_<id> <args>
 *
 * Accepts either:
 *   - key=value pairs: interval=4 freq=DAILY byhour=9,17 tzid=America/New_York
 *   - natural shorthand: "every 4h", "every 30m", "every 2d", "every 1w"
 *   - no args: shows usage + current rule
 *
 * Merges changes into the existing rule, validates, rearms the timer.
 */
function handleScheduleEdit(event, core, scheduleTaskId, args) {
    const found = findScheduledTask(core.specialData, scheduleTaskId)
    if (!found) {
        return reply(event.chatId, `Unknown scheduled task: \`${esc(scheduleTaskId)}\``, event.threadId)
    }
    ...
    if (task.state === "cancelled" || task.state === "completed") {
        return reply(event.chatId, `Cannot edit — task is \`${esc(task.state)}\`.`, event.threadId)
    }
    const oldRule = task.rule ?? {}

    if (!args) {
        const lines = [
            `Edit scheduled task ${scheduleTaskId}`,
            ``,
            `Current rule:`,
            JSON.stringify(oldRule, null, 2),
            ``,
            `Usage:`,
            `/schedule_edit_${scheduleTaskId} every 4h`,
            `/schedule_edit_${scheduleTaskId} interval=6`,
            `/schedule_edit_${scheduleTaskId} freq=DAILY byhour=9`,
            ``,
            `Shorthand: every Nh/Nm/Nd/Nw`,
            `Key=value: freq, interval, byhour, byminute, byday, bymonth, bymonthday, tzid, count, until`,
        ]
        return replyPlain(replyToFromEvent(event, "schedule"), lines.join("\n"))
    }

    // Try natural shorthand: "every 4h", "every 30m", etc.
    const naturalMatch = /^every\s+(\d+)\s*(h|hr|hrs|hour|hours|m|min|mins|minute|minutes|d|day|days|w|week|weeks|s|sec|secs|second|seconds)$/i.exec(args)
    const patch = {}
    if (naturalMatch) {
        const n = parseInt(naturalMatch[1], 10)
        ...
        for (const part of parts) {
            ...
            if (eqIdx < 1) {
                return reply(event.chatId, `Invalid argument: \`${esc(part)}\`. Use \`key=value\` format or \`every Nh\`.`, event.threadId)
            }
            const key = part.slice(0, eqIdx).toLowerCase()
            const val = part.slice(eqIdx + 1)
            if (key === "freq") {
                patch.freq = val.toUpperCase()
            } else if (key === "interval") {
                patch.interval = parseInt(val, 10)
            } else if (key === "tzid") {
                patch.tzid = val
            } else if (key === "count") {
                patch.count = parseInt(val, 10)
            } else if (key === "until") {
                patch.until = val
            } else if (key === "byhour") {
                patch.byhour = val.split(",").map(Number)
            } else if (key === "byminute") {
                patch.byminute = val.split(",").map(Number)
            } else if (key === "byday") {
                patch.byday = val.split(",")
            } else if (key === "bymonth") {
                patch.bymonth = val.split(",").map(Number)
            } else if (key === "bymonthday") {
                patch.bymonthday = val.split(",").map(Number)
            } else {
                return reply(event.chatId, `Unknown rule key: \`${esc(key)}\``, event.threadId)
            }
        }
    }

    if (Object.keys(patch).length === 0) {
        return reply(event.chatId, `No changes parsed from: \`${esc(args)}\``, event.threadId)
    }

    const newRule = { ...oldRule, ...patch }
    const check = validateRule(newRule)
    if (!check.ok) {
        return reply(event.chatId, `Invalid rule after edit: ${esc(check.error)}`, event.threadId)
    }

    dbg("CHAT-USER", `editing scheduled task ${scheduleTaskId} rule: ${JSON.stringify(patch)}`)
    return {
        stateChanges: {
            specialData: {
                scheduledTaskByChatId: {
        ...
        effects: [
            { type: "schedule_timer_set", chatId: found.chatId, scheduleTaskId, rule: newRule, from: new Date().toISOString() },
            {
                type: "write_file",
                path: paths.scheduledTaskDir(scheduleTaskId) + "/rule.json",
                content: JSON.stringify(newRule, null, 2),
            },
            {
                type: "cold_append",
                stream: "scheduled-tasks",
                entry: { scheduleTaskId, chatId: found.chatId, event: "rule_edited", oldRule, newRule },
            },
            {
                type: "send_text_to_user",
                chatId: event.chatId,
                text: `Updated rule for ${scheduleTaskId}:\n${JSON.stringify(newRule, null, 2)}\n\nTimer rearmed.`,
                options: { ...(event.threadId != null ? { message_thread_id: Number(event.threadId) } : {}) },
            },
        ],
    }
