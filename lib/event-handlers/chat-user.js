...
// Typing the code char-by-char is slow, and the TUI needs a beat after it to
// swap the panel out, so the confirming peek waits this long.
const LOGIN_CODE_PEEK_DELAY_MS = 5000
...
function handleLoginCode(event, core, code, pendingLogin) {
    ...
    return {
        stateChanges: {},
        effects: [
            // This already submits — a second Enter would land on whatever
            // the TUI shows next and cancel it.
            { type: "send_raw_input_to_claude", sessionId, text: code },
            {
                type: "set_timer",
                delayMs: LOGIN_CODE_PEEK_DELAY_MS,
                event: {
                    ...
                    userId: event.userId,
                    username: event.username ?? null,
                    messageId: `login_peek_${Date.now()}`,
                    text: "/peek",
                    ts: Date.now(),
                },
            },
