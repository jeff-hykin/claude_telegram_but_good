...
/**
 * Build channel event content, prepending reply-to context as XML when present.
 */
function buildChannelContent(messageText, event) {
    if (event.replyToText) {
        return `<reply_to message_id="${event.replyToMessageId}">\n${event.replyToText}\n</reply_to>\n${messageText}`
    }
    return messageText
}
