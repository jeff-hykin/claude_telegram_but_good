...
export default async function handle(event, core) {
    ...
    if (/^\/task_status(?:@\w+)?$/i.test(trimmed)) {
        ...
        return wrap(reply(rt, "No active task found for this topic. Use <code>/task_status_&lt;id&gt;</code> to specify one."))
    }
    ...
    if (isCommandCenter && event.threadId) {
        if (ccTopicSessionId && core.chatSessions?.[ccTopicSessionId]) {
            ...
            return wrap({
                stateChanges: {},
                effects: [sendEffect(event.replyTo, "No session is attached to this topic. Use /refresh to spawn one.", { parse_mode: "HTML" })],
            })
        } else {
            ...
            return wrap({
                stateChanges: {},
                effects: [sendEffect(event.replyTo, `Session <code>${esc(ccTopicSessionId)}</code> has disconnected. Use /refresh to respawn it.`, { parse_mode: "HTML" })],
            })
        }
    }
...
function enqueueWaitingMessage(event, core, trimmed) {
    ...
    return {
        ...
        effects: [sendEffect(rt, userText, { parse_mode: "HTML" })],
    }
}
...
function handleAttachment(event, core) {
    ...
    if (!session?._conn) {
        return reply(attachRt, `Session <code>${esc(targetId)}</code> is disconnected — the Claude process is gone. Start a new one with /new.`)
    }
...
function handleClose(event, core, targetId) {
    ...
    if (!target) {
        return reply(rt, `Session <code>${esc(targetId)}</code> not found.`)
    }
...
function handleConnect(event, core, targetId) {
    ...
    if (!session._conn) {
        return reply(rt, `Session <code>${esc(targetId)}</code> is disconnected.`)
    }
    ...
    return {
        ...
        effects: [sendEffect(rt,
            `Connected <code>${esc(targetId)}</code> (${esc(title)}) to this topic.${oldSessionId ? `\nUnbound previous session <code>${esc(oldSessionId)}</code>.` : ""}`,
            { parse_mode: "HTML" },
        )],
    }
}
...
function handleShellMessage(event, core, body) {
    ...
    if (parsed.kind === "empty") {
        return reply(replyTo, `<i>cwd: ${esc(currentCwd)}</i>`)
    }
    ...
    if (parsed.kind === "cd-home" || parsed.kind === "cd") {
        ...
        try {
            ...
            return reply(replyTo, `<b>cd:</b> ${esc(target)}: ${esc(msg)}`)
        }
        if (!stat.isDirectory) {
            return reply(replyTo, `<b>cd:</b> ${esc(target)}: not a directory`)
        }
        return {
            stateChanges: {
                chatState: {
            ...
            effects: [sendEffect(replyTo, `<i>cwd: ${esc(target)}</i>`, { parse_mode: "HTML" })],
        }
    }
...
function handleTaskCreate(event, core, description) {
    ...
    if (!description) {
        ...
        return reply(rt0, "Usage: <code>/task &lt;description&gt;</code>")
    }
    ...
    return {
        ...
        effects: [
            ...
            ...activation.effects,
            sendEffect(rt, `/task_cancel_${taskId}`, { parse_mode: "HTML" }),
            {
                type: "cold_append",
                stream: "long-tasks",
                entry: {
...
function handleTaskStatus(event, core, taskId) {
    ...
    if (!task) {
        return reply(rt, `Task <code>${esc(taskId)}</code> not found.`)
    }
...
function handleTaskView(event, core, taskId) {
    ...
    if (!task.definition) {
        return reply(rt, `Task <code>${esc(taskId)}</code> has no definition yet (still in <code>defining</code> state).`)
    }
...
function handleTaskUpdate(event, core, taskId, body) {
    ...
    if (!newDefinition) {
        ...
        if (!task.definition) {
            return reply(rt, `Task <code>${esc(taskId)}</code> has no definition yet. Reply with <code>/task_update_${esc(taskId)} &lt;new text&gt;</code> to set one.`)
        }
        const def = task.definition.slice(0, 3000)
        const truncated = task.definition.length > def.length ? "\n\n[truncated]" : ""
        const text =
...
function handleTaskResume(event, core, taskId) {
    ...
    const workerSessionId = task.workerSessionId
    if (!workerSessionId) {
        return reply(rt, `Task <code>${esc(taskId)}</code> has no worker session recorded. Cannot resume.`)
    }
    const workerSession = core.chatSessions?.[workerSessionId]
    if (!workerSession) {
        return reply(rt, `Worker session <code>${esc(workerSessionId)}</code> for this task is gone. Cannot resume.`)
    }
...
function handleScheduleCreate(event, core, description) {
    ...
    if (!description) {
        return reply(rt, "Usage: <code>/schedule &lt;description&gt;</code>")
    }
    // In CC mode, use the topic's session. In DM, use focused.
    let focusedId = null
    if (event.threadId) {
    ...
    return {
        ...
        effects: [
            ...
            sendEffect(rt, `Drafting scheduled task <code>${esc(scheduleTaskId)}</code> — clarify with the agent, then it'll be locked.`, { parse_mode: "HTML" }),
        ],
    }
}
...
function handleScheduleEdit(event, core, scheduleTaskId, args) {
    ...
    if (task.state === "cancelled" || task.state === "completed") {
        return reply(event.chatId, `Cannot edit — task is <code>${esc(task.state)}</code>.`, event.threadId)
    }
    const oldRule = task.rule ?? {}

    if (!args) {
        const lines = [
            `<b>Edit scheduled task <code>${esc(scheduleTaskId)}</code></b>`,
            ``,
            `Current rule:`,
            `<pre>${esc(JSON.stringify(oldRule, null, 2))}</pre>`,
            ``,
            `Usage:`,
            `<code>/schedule_edit_${esc(scheduleTaskId)} every 4h</code>`,
            `<code>/schedule_edit_${esc(scheduleTaskId)} interval=6</code>`,
            `<code>/schedule_edit_${esc(scheduleTaskId)} freq=DAILY byhour=9</code>`,
            ``,
            `Shorthand: <code>every Nh/Nm/Nd/Nw</code>`,
    ...
    if (naturalMatch) {
        ...
        for (const part of parts) {
            ...
            if (eqIdx < 1) {
                return reply(event.chatId, `Invalid argument: <code>${esc(part)}</code>. Use <code>key=value</code> format or <code>every Nh</code>.`, event.threadId)
            }
            ...
            if (key === "freq") {
                ...
                return reply(event.chatId, `Unknown rule key: <code>${esc(key)}</code>`, event.threadId)
            }
        }
    }

    if (Object.keys(patch).length === 0) {
        return reply(event.chatId, `No changes parsed from: <code>${esc(args)}</code>`, event.threadId)
    }
    ...
    return {
        ...
        effects: [
            ...
            },
            {
                type: "send_text_to_user",
                chatId: event.chatId,
                text: `Updated rule for <code>${esc(scheduleTaskId)}</code>:\n<pre>${esc(JSON.stringify(newRule, null, 2))}</pre>\n\nTimer rearmed.`,
                options: { parse_mode: "HTML", ...(event.threadId != null ? { message_thread_id: Number(event.threadId) } : {}) },
            },
        ],
    }
