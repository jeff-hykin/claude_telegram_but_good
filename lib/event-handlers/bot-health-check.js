    ...
    if (health.pollingDied) {
        ...
        try {
            ...
            if (ok) {
                ...
                // Notify the command center so the user knows it happened.
                const cc = core.chatState?.commandCenter
                if (cc?.chatId) {
                    const threadId = findCbgThreadId(cc)
                    effects.push({
                        type: "send_text_to_user",
                        replyTo: { chatId: cc.chatId, threadId },
                        text: `⚠️ Grammy polling loop died and was auto-restarted (restart #${health.restartCount}).`,
                    })
                }
            } else {
            ...
            if (ok) {
                ...
                if (cc?.chatId) {
                    ...
                    effects.push({
                        type: "send_text_to_user",
                        replyTo: { chatId: cc.chatId, threadId },
                        text: `⚠️ Grammy went silent for ${silentMin}min — auto-restarted (restart #${health.restartCount}).`,
                    })
                }
            } else {
