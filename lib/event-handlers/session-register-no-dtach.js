...
export default function handle(event, core) {
    ...
    const text = [
        `⚠️ Session \`${escapeMarkdown(sessionId)}\` registered without dtach in its ancestry.`,
        `*cwd:* \`${escapeMarkdown(cwd)}\``,
        `*branch:* \`${escapeMarkdown(gitBranch)}\``,
        ``,
        `_/peek, /cancel, /pause, /resume won't work for it._`,
        ``,
