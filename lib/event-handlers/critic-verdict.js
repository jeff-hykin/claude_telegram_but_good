...
const { getScheduleCriticMaxRetries } = await versionedImport("../config-manager.js", import.meta)
 ...
 */
/**
 * Produce the effect that reports a critic status to the user.
 *
 * By default, edits the "Critic running…" message in place (low-priority
 * in the rate limiter — fine for interim updates). When `important` is
 * true, sends a NEW message instead (high-priority) so it can't be
 * silently dropped by the rate limiter. Use important=true for final
 * verdicts like certification and terminal revision failures.
 */
function criticStatusEffect(task, chatId, text, core, { important = false } = {}) {
    if (!important && task?.criticRunningMessageId && task?.criticRunningChatId != null) {
        return {
    ...
    const replyTo = replyToForSession(
        task?.workerSessionId,
        core,
        "critic-verdict",
        chatId,
    )
    return sendEffect(replyTo, text, { parse_mode: "Markdown" })
}
...
function handleCertified(event, core, task) {
    ...
    if (chatId) {
        effects.push(criticStatusEffect(
            task,
            chatId,
            `\u2705 Task \`${escapeMarkdown(taskId)}\` certified by critic.`,
            core,
            { important: true },
        ))
    }
...
function handleRevisions(event, core, task) {
    ...
    if (chatId) {
        effects.push(criticStatusEffect(
            task,
            chatId,
            `\u26A0\uFE0F Task \`${escapeMarkdown(taskId)}\`: critic requested revisions. Worker is re-drafting.`,
            core,
        ))
    }
...
function handleRetry(event, core, task) {
    ...
    if (chatId) {
        ...
        effects.push(criticStatusEffect(
            task,
            chatId,
            `\u274C Task \`${escapeMarkdown(taskId)}\`: critic failed after 3 attempts. ` +
            `Last error: \`\`\`\n${escapeMarkdown(detailsText)}\n\`\`\`` +
            `Please intervene.`,
            core,
            { important: true },
        ))
...
/**
 * Scheduled-run verdict branch. Differs from the long-task branch:
 *   - certified: kill the worker, send a plain telegram message with
 *     the report body, enqueue scheduled_task_run_complete.
 *   - revisions: write revision_request.md into the run dir, delete
 *     old report.md, inject revision feedback into the worker via
 *     dtach -p (with the log-stability guard inside the effect).
 *   - indecisive/error: retry up to schedule_critic_max_retries, then
 *     give up and enqueue scheduled_task_run_complete errored.
 *   - dry-run: just log.
 */
function handleScheduledRunVerdict(event, core) {
    ...
    if (!found) {
        dbg("CRITIC-VERDICT", `scheduled task ${scheduleTaskId} not found for verdict ${verdict}`)
        return null
    }

    if (verdict === "certified") {
        const reportPath = join(paths.scheduledTaskRunDir(scheduleTaskId, runIso), "report.md")
        let body = ""
        try { body = Deno.readTextFileSync(reportPath) }
        catch (e) { dbg("CRITIC-VERDICT", "read report.md for certified scheduled run:", e) }
        dbg("CRITIC-VERDICT", `scheduled run certified: ${scheduleTaskId} runIso=${runIso}`)
        return {
            effects: [
    ...
    if (verdict === "revisions" || verdict === "anomaly" || verdict === "clarification_needed") {
        const runDir = paths.scheduledTaskRunDir(scheduleTaskId, runIso)
        const revPath = join(runDir, "revision_request.md")
        const reportPath = join(runDir, "report.md")
        const revText = details || "(no details supplied by critic)"
        dbg("CRITIC-VERDICT", `scheduled run revisions: ${scheduleTaskId} runIso=${runIso}`)
        return {
            effects: [
                { type: "write_file", path: revPath, content: revText },
                { type: "delete_file", path: reportPath },
                {
                ...
                },
                {
                    type: "cold_append",
                    stream: "scheduled-tasks",
                    entry: { scheduleTaskId, chatId, event: "run_revisions", runIso, attempt },
                },
            ],
        }
    }

    if (verdict === "indecisive" || verdict === "error") {
        const maxRetries = getScheduleCriticMaxRetries()
        if (attempt < maxRetries) {
            dbg("CRITIC-VERDICT", `scheduled run retry ${attempt + 1}/${maxRetries}: ${scheduleTaskId}`)
            return {
                effects: [
                    {
                        type: "spawn_critic",
        ...
        dbg("CRITIC-VERDICT", `scheduled run giving up after ${attempt} attempts: ${scheduleTaskId}`)
        return {
            effects: [
                { type: "scheduled_task_worker_kill", scheduleTaskId, runIso },
                {
                    type: "cold_append",
                    stream: "scheduled-tasks",
                    entry: { scheduleTaskId, chatId, event: "run_errored", runIso, verdict, attempt },
                },
            ],
            followUpEvents: [
                {
                    ...
                    chatId, scheduleTaskId, runIso,
                    status: "errored",
                    summary: `critic gave up after ${attempt} attempts: ${(details ?? verdict).toString().slice(0, 500)}`,
                },
            ],
        }
    }

    dbg("CRITIC-VERDICT", `scheduled run unknown verdict: ${verdict}`)
    return null
}

export default function handle(event, core) {
    // Scheduled-task runs travel through the same critic_verdict
    // pipeline, but with a `scheduledRun` wrapper instead of a taskId.
    // They take a completely different branch — no long-task state,
    // no criticStatusEffect, no escalation-via-worker notification.
