...
//   "revisions" / "anomaly"-> tell the worker to re-read revisions and try
//                             again, transition state back to "in_progress".
...
//
// v1 simplification: revisions archival (rename requested_revisions.md to
// revisions/requested_revisions.<ts>.md and delete report.md) is NOT done
// here — handlers can't perform disk operations directly and we don't have
// file move/delete effects yet. The critic's output files just stay on disk
...
function handleCertified(event, core, task) {
    ...
    if (workerSessionId) {
        effects.push({
            ...
            text:
                `[long task ${taskId} — certified]\n` +
                `The critic has certified this task as complete. ` +
                `Please notify the user on Telegram with a short summary of what was accomplished.`,
        })
    } else {
        dbg("CRITIC-VERDICT", `certified ${taskId} but no workerSessionId on task`)
    }
    ...
    if (workerSessionId) {
        ...
        effects.push({
            ...
            text:
                ...
                `${taskDirAbs}/requested_revisions.md ` +
...
function handleClarification(event, core, task) {
    const { taskId, chatId } = event
    dbg("CRITIC-VERDICT", `clarification_needed taskId=${taskId} chatId=${chatId}`)
    ...
    if (chatId) {
        effects.push({
            ...
            chatId,
            text:
                `Task <code>${escapeHtml(taskId)}</code>: ` +
                `the critic needs clarification on the definition of done.`,
            options: { parse_mode: "HTML" },
        })
    } else {
        dbg("CRITIC-VERDICT", `clarification_needed ${taskId} but no chatId`)
    }

    effects.push({
        type: "cold_append",
        stream: "long-tasks",
        entry: { event: "clarification_needed", taskId, chatId },
    })
...
function handleRetry(event, core, task) {
    ...
    if (chatId) {
        ...
        effects.push({
            ...
            text:
                `Task <code>${escapeHtml(taskId)}</code>: ` +
                `critic failed after 3 attempts. ` +
...
export default function handle(event, core) {
    ...
    switch (verdict) {
        ...
        case "clarification_needed":
            return handleClarification(event, core, task)
        case "indecisive":
        case "error":
