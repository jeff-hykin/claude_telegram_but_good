...
export default function handle(event, core) {
    ...
    if (action === "askAgentToSendChatMessage") {
        ...
        if (!task) {
            ...
            const criticOpts = { parse_mode: "HTML" }
            const cc = core.chatState?.commandCenter ?? {}
            const criticThreadId = cc.topicMap?.[event.sessionId]
            if (criticThreadId != null) { criticOpts.message_thread_id = Number(criticThreadId) }
            effects.push({
