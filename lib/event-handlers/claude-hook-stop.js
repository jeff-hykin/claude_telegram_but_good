...
const { makeReplyTo, replyToForSession } = await versionedImport("../pure/reply-to.js", import.meta)
...
export default function handle(event, core) {
    ...
    if (action === "askAgentToSendChatMessage") {
        ...
        if (!task) {
            ...
            const criticReplyTo = replyToForSession(event.sessionId, core, "hook-stop:critic-running")
                ?? makeReplyTo({ chatId, threadId: null, setBy: "hook-stop:critic-running" })
            effects.push({
                type: "send_text_to_user",
                replyTo: criticReplyTo,
                text:
                    `Critic running on <code>${esc(taskId)}</code> ` +
                    `(report.md received; this usually takes 30–90 s)…`,
                options: { parse_mode: "HTML" },
                stashCriticMessageIdOnTask: { chatId, taskId },
            })
            effects.push({
                type: "cold_append",
                stream: "long-tasks",
