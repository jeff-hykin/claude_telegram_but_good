...
const NUDGE_REPORT_THRESHOLD = 1 // nudge on first idle stop with no report
const TASK_CHECKIN_MS = 30 * 60 * 1000 // 30 minutes between check-in nudges
    ...
    if (action === "askAgentToSendChatMessage") {
        ...
        if (!task) {
            ...
            if (shouldNudge) {
                ...
                // Reset the 30-minute check-in timer after nudging.
                effects.push({
                    type: "set_timer",
                    delayMs: TASK_CHECKIN_MS,
                    event: { type: "task_checkin", sessionId: event.sessionId, taskId },
                })
                effects.push({
                    type: "cold_append",
                    stream: "long-tasks",
