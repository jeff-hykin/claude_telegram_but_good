...
// 20 minutes for the task-report nudge. set_timer has no cancellation, so
// every Stop stacks another timer; the fire handler (stop-nudge-fire.js)
// additionally rate-limits to at most one task nudge per 20 min via the
// task's lastNudgeAt, so stacked timers can't double-fire.
const TASK_NUDGE_DELAY_MS = 20 * 60 * 1000
...
export default function handle(event, core) {
    ...
    const sessionPatch = {
        ...
        lastActive: ts,
        status: "idle",
        // A listening session is unlocked for exactly one turn when
        // someone addresses the bot; that turn is over, so it goes quiet.
        listenUnlockedAt: undefined,
        // Memory hooks fire at most once per turn (lib/memory-hooks.js).
        // The turn is over, so every hook is armed again.
        memoryHooksFired: undefined,
    }

    // Claude's transcript path only reaches us through hook payloads, and
    // it's stable for the life of the session — record it the first time
    // we see one so the context-usage read has a file to open.
    if (typeof event.transcriptPath === "string" && event.transcriptPath !== session.transcriptPath) {
        sessionPatch.transcriptPath = event.transcriptPath
    }
    ...
    if (action === "askAgentToSendChatMessage") {
        ...
        if (!task) {
            ...
            sessionPatch.pendingNudgeAction = "none"
        } else if (reportMdExists(taskId) && core.activeCritics?.has(taskId)) {
            // A critic for this task is already in flight (from an earlier
            // Stop or a verdict-driven retry). Skip the spawn AND the
            // "Critic running…" toast so the user doesn't see a ghost
            // notification on every Stop while the first run is still
            // working. The critic-subprocess effect has the same guard;
            // checking here keeps the user-facing message in sync.
            dbg("HOOK-STOP", `taskCheck: critic for ${taskId} already in flight; skipping spawn`)
            sessionPatch.pendingNudgeAction = "none"
        ...
        if (next._source !== "cli") {
            ...
            if (queReplyTo.chatId) {
                ...
                effects.push(sendEffect(
                    queReplyTo,
                    `Queued message delivered: _${esc(preview)}_${remaining.length > 0 ? `\n(${remaining.length} still queued)` : ""}`,
                    { parse_mode: "Markdown" },
                ))
            }
        }
