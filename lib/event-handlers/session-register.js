...
export default function handle(event, core) {
    ...
    // If the session has no explicit title, derive one from cwd + branch
    // so command center topics and /list always show something useful.
    const effectiveTitle = session.title ?? deriveTitle(session)
    ...
    const sessionTitle = session.title ?? deriveTitle(session)
