...
export default function handle(event, core) {
    ...
    // Drain queued inbound messages onto the newly-registered session.
    // Messages with a `targetSessionId` are only delivered to that
    // specific session. Messages without a target (legacy) are delivered
    // to whoever becomes focused.
    {
        ...
        if (queue.length > 0) {
            ...
            for (const entry of queue) {
                if (entry.targetSessionId === id) {
                    // Targeted at this session — deliver regardless of focus
                    toDeliver.push(entry)
                } else if (!entry.targetSessionId && becameFocused) {
                    // No target — legacy behavior: deliver to newly focused
                    toDeliver.push(entry)
                } else {
                    toKeep.push(entry)
                }
            ...
            if (toDeliver.length > 0) {
                dbg("SESSION-REG", `draining ${toDeliver.length} queued message(s) to ${id} (${toKeep.length} kept)`)
                // Set lastInbound + activate the nudge system so the
                // session gets reminded to reply if it goes idle.
                const lastEntry = toDeliver[toDeliver.length - 1]
                const now = Date.now()
                const nextAgentRequest = 1
                stateChanges.chatSessions = {
                    ...(stateChanges.chatSessions ?? {}),
                    [id]: {
                        ...(stateChanges.chatSessions?.[id] ?? {}),
                        lastInbound: {
                            messageId: lastEntry.meta?.message_id ?? null,
                            chatId: lastEntry.meta?.chat_id ?? null,
                            ts: now,
                            text: (lastEntry.content ?? "").slice(0, 500),
                        },
                        status: "working",
                ...
                for (const entry of toDeliver) {
                    effects.push({
                        ...
                        meta: entry.meta,
                        _queueDrain: true,
                        _chatId: entry.meta?.chat_id ?? null,
                        _threadId: entry.threadId ?? null,
                    })
                }
                stateChanges.chatState = { ...(stateChanges.chatState ?? {}), messageQueue: toKeep }
            }
        }
    }
