...
//
// Periodic screen-buffer snapshotter. Reads `tail -c N` of each live
// session's dtach log, hashes it, and appends `{ hash, ts }` to a ring
// buffer on `chatSessions[sid].screenBufferRecord`. Used by stall-check.js
...
//
// Disk side effect: `Deno.readTextFileSync` (async-friendly) per live
// session. At default 20s interval and <50 sessions that's under 3 reads
// per second of ~300 bytes each — a fraction of a ms of CPU.
// ---------------------------------------------------------------------------
