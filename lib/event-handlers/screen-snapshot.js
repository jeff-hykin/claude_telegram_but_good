// ---------------------------------------------------------------------------
// lib/event-handlers/screen-snapshot.js
//
...
const { detectPrompts } = await versionedImport("../pure/tui-prompt-detector.js", import.meta)
const { renderTui } = await versionedImport("../pure/tui-render.js", import.meta)
...
export default function handle(event, core) {
    ...
    for (const [sid, session] of Object.entries(sessions)) {
        ...
        // Prompt detection: render the screen and scan for blocking
        // TUI prompts. Runs on ALL sessions (including idle) since a
        // session stuck on a TUI prompt appears idle but is actually
        // blocked. Only check if the screen has content and the hash
        // matches the previous snapshot (screen isn't changing).
        const prevRecord = Array.isArray(session.screenBufferRecord)
            ? session.screenBufferRecord
            : []
        const prevHash = prevRecord.length > 0
            ? prevRecord[prevRecord.length - 1].hash
            : null
        if ((prevHash === hash || !isInFlight) && tail.length > 0) {
            try {
                const rendered = renderTui(tail, { width: 80, height: 30, ansi: false, trim: true })
                const prompts = detectPrompts(rendered)
                ...
                dbg("SCREEN-SNAP", `prompt detection failed for ${sid}:`, e)
            }
        }
    }
