...
export default function handle(event, core) {
    ...
    for (const [sid, session] of Object.entries(sessions)) {
        ...
        // Only snapshot sessions that are actually in flight. Idle
        // sessions have no stall_check watching their ring buffer, so
        // appending to it would be pure overhead (read syscall + state
        // patch + persistence schedule per tick per idle session). A
        // session that's never been in a waiting state has no `status`
        // field at all — treat that as idle too.
        if (session.status !== "working" && session.status !== "frozen") {
            continue
        }
