...
export default function handle(event, core) {
    ...
    return {
        ...
        effects: [
            ...
            sendEffect(
                replyToForSession(event.sessionId, core, "long-task-def-submitted", chatId),
                `Task \`${esc(taskId)}\` confirmed to be understood by agent` +
                `\n\n` +
                taskCommandLinks(taskId),
                { parse_mode: "Markdown" },
            ),
            // Schedule the first 30-minute check-in timer.
            {
                type: "set_timer",
                delayMs: 30 * 60 * 1000,
