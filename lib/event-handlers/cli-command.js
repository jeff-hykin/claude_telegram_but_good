...
export default function handle(event, core) {
    ...
    if (kind === "tell_session") {
        ...
        // Build the message content with reply hint. The hint MUST use the
        // explicit `inbox:` prefix in the target — the bare-fuzzy resolver
        // tries session ID → topic name → title BEFORE falling back to
        // inbox, so when the inbox name overlaps a topic name (e.g. gre's
        // `--from gre-jeff-fix-rosnav8` matches the bound topic of the
        // same name), a bare `target="..."` reply gets routed to the
        // topic — never reaches the parked ask_sync waiter on the inbox.
        const fromLabel = replyToInbox ? `inbox:${replyToInbox}` : "CLI"
        const replyHint = replyToInbox
            ? ` — reply with: tell_session target="inbox:${replyToInbox}" text="..."`
            : ""
    ...
    if (kind === "ask_sync") {
        ...
        // `target="inbox:..."` is load-bearing: bare resolution tries
        // session ID → topic → title before inbox, so when --from
        // overlaps a topic name (gre's per-branch inbox often does)
        // a bare reply gets routed to the topic and the parked ask_sync
        // waiter on the inbox never wakes.
        const content = `[from inbox:${replyToInbox} — reply with: tell_session target="inbox:${replyToInbox}" text="..."]\n${text}`
