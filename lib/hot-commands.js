...
// Fisher-Yates shuffle of tip indices. Exhausts the full list before
// repeating, so you never see the same tip back-to-back.
let shuffledTipIndices = []
let shufflePos = 0

function refillShuffle() {
    shuffledTipIndices = allTips.map((_, i) => i)
    for (let i = shuffledTipIndices.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1))
        ;[shuffledTipIndices[i], shuffledTipIndices[j]] = [shuffledTipIndices[j], shuffledTipIndices[i]]
    }
    shufflePos = 0
}
...
export function getRandomTip() {
    ...
    if (shufflePos >= shuffledTipIndices.length || shuffledTipIndices.length !== allTips.length) {
        refillShuffle()
    }
    return allTips[shuffledTipIndices[shufflePos++]]
}
