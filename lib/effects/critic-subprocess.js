...
Detecting a worker that is shirking:
Watch for a report that tries to wrap up before the work is actually done. Tell-tale signs: it stops with done-conditions unmet; it makes excuses about being "blocked" on things it could resolve itself; it presents partial progress as if it were completion; or it spends the report dwelling on how much it already did while substantial required work remains. The definition of done is the contract — effort already spent does NOT count toward it, and a tired or rationalizing worker still owes the remaining work.

When you issue "Revisions:" in this situation, ALSO do these two things inside the Revisions body:
  1. List the concrete remaining items still required by the definition of done — each as a direct, actionable instruction the worker can complete alone.
  2. Tell the worker to reset its context and continue as a fresh SUCCESSOR. Instruct it, in these words, to invoke the "self_compact" skill (or the "self_clear" skill if its context is badly polluted with rationalizing/excuse-making) and then treat itself as a new worker who has INHERITED all the already-completed work, whose only remaining job is to finish the items you listed. The successor should not re-litigate, re-explain, or re-do prior work — only complete what remains. (If it uses self_clear, remind it to write the remaining items to a durable handoff file first, since /clear erases everything.)

This reset-and-succeed guidance stays inside the "Revisions:" body — do not change the output format or add a third token.
 ...
 */
async function spawnAndDrainCritic({ args, cleanEnv, hardTimeoutMs, logLabel }) {
    const POLL_MS = 500
    ...
    const stderrChunks = []
    const child = new Deno.Command("claude", {
        args, env: cleanEnv, clearEnv: true,
    ...
    while (!exited) {
        if (Date.now() - begin > hardTimeoutMs) {
...
async function runCriticCore({ dir, attempt, logLabel }) {
    ...
    let details = ""
    try {
        const primaryArgs = ["--model", model, "--fallback-model", fallbackModel, ...baseArgs]
        let { stdoutText, stderrText } = await spawnAndDrainCritic({
            args: primaryArgs,
            cleanEnv,
        ...
        // Empty-stdout fallback: claude-cli sometimes returns 0 bytes
        // (root cause unclear — possibly a transient API issue, possibly
        // the model used tools and never emitted a final assistant
        // message). Retry once on the fallback model so a single bad
        // primary-model run doesn't burn an attempt slot. Reproducible
        // by-hand runs of the same prompt on the same model usually
        // succeed, so a single retry is enough in practice.
        if (stdoutText.length === 0) {
            dbg("CRITIC-SUB", `empty stdout from primary model ${model} (${logLabel}); retrying with ${fallbackModel}`)
            const fallbackArgs = ["--model", fallbackModel, ...baseArgs]
            const r2 = await spawnAndDrainCritic({
                args: fallbackArgs,
                cleanEnv,
                hardTimeoutMs: HARD_TIMEOUT_MS,
                logLabel: `${logLabel} (fallback)`,
            })
            if (r2.stdoutText.length > 0) {
                dbg("CRITIC-SUB", `fallback model produced ${r2.stdoutText.length}B (${logLabel})`)
                stdoutText = r2.stdoutText
                stderrText = (stderrText || "") +
                    `\n--- empty stdout from primary; retried on fallback ${fallbackModel} ---\n` +
                    r2.stderrText
            } else {
                dbg("CRITIC-SUB", `fallback model also produced 0 bytes (${logLabel})`)
                stderrText = (stderrText || "") +
                    `\n--- fallback ${fallbackModel} also produced 0 bytes ---\n` +
                    r2.stderrText
            }
        }
...
export async function spawnCriticSubprocess(effect, core) {
    ...
    // Per-task spawn dedup. The hook-stop handler can re-fire before
    // the in-flight critic completes; without this guard we'd run
    // parallel claude CLIs against the same task and race on
    // session/cache state. claude-hook-stop.js does the same check
    // before emitting the "Critic running…" message so the user
    // doesn't see ghost notifications either.
    if (!core.activeCritics) { core.activeCritics = new Set() }
    if (core.activeCritics.has(taskId)) {
        dbg("CRITIC-SUB", `spawn_critic: ${taskId} already in flight (attempt ${attempt}); skipping`)
        return
    }
    ...
    ;(async () => {
        try {
            ...
            core.activeCritics.delete(taskId)
        }
...
async function spawnCriticForScheduledRun(effect, core) {
    ...
    // Per-run dedup. Same rationale as the long-task path; key includes
    // runIso so retries against the SAME run are deduped but separate
    // runs of the same schedule can still race independently.
    if (!core.activeCritics) { core.activeCritics = new Set() }
    const dedupKey = `sched:${scheduleTaskId}:${runIso}`
    if (core.activeCritics.has(dedupKey)) {
        dbg("CRITIC-SUB", `spawn_critic: ${dedupKey} already in flight (attempt ${attempt}); skipping`)
        return
    }
    ...
    core.activeCritics.add(dedupKey)
    ;(async () => {
        try {
            const { verdict, details, elapsedMs } = await runCriticCore({
                dir: runDir, attempt,
            ...
            core.activeCritics.delete(dedupKey)
        }
