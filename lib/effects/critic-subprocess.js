 ...
 *
 * When the critic ignores the format requirement, a fallback heuristic
 * scans the body for strong acceptance or revision signals. This is
 * less reliable than the prefix format but prevents giving up on
 * substantive responses that just missed the format.
 */
function parseCriticStdout(stdout) {
    const trimmed = (stdout ?? "").replace(/^\s+/, "")

    // ── Primary: strict prefix match ────────────────────────────────
    ...
    // ── Secondary: prefix appears mid-text (critic added preamble) ──
    const midAccepted = /\n\s*Accepted\s*:\s*\n/i.exec(trimmed)
    if (midAccepted) {
        dbg("CRITIC-SUB", "found Accepted: mid-text — treating as certified")
        return { verdict: "certified", body: trimmed.slice(midAccepted.index + midAccepted[0].length).trim() }
    }
    const midRevisions = /\n\s*Revisions?\s*:\s*\n/i.exec(trimmed)
    if (midRevisions) {
        dbg("CRITIC-SUB", "found Revisions: mid-text — treating as revisions")
        return { verdict: "revisions", body: trimmed.slice(midRevisions.index + midRevisions[0].length).trim() }
    }

    // ── Fallback: keyword heuristic when format is completely ignored ─
    const lower = trimmed.toLowerCase()
    const revisionSignals = [
        "needs revision", "not yet complete", "must be fixed",
        "does not meet", "doesn't meet", "fails to", "missing",
        "not satisfied", "incomplete", "needs to be", "should be fixed",
        "not passing", "fix required", "must address",
    ]
    const acceptSignals = [
        "all criteria met", "all requirements met", "satisfies the definition",
        "correctly", "work is complete", "meets the definition",
        "all conditions", "definition of done is satisfied",
        "confirmed", "all checks pass", "properly",
    ]

    let revisionScore = 0
    let acceptScore = 0
    for (const s of revisionSignals) {
        if (lower.includes(s)) { revisionScore++ }
    }
    for (const s of acceptSignals) {
        if (lower.includes(s)) { acceptScore++ }
    }

    if (acceptScore > 0 && acceptScore > revisionScore) {
        dbg("CRITIC-SUB", `fallback heuristic: accepted (accept=${acceptScore}, revision=${revisionScore})`)
        return { verdict: "certified", body: trimmed, heuristic: true }
    }
    if (revisionScore > 0 && revisionScore > acceptScore) {
        dbg("CRITIC-SUB", `fallback heuristic: revisions (accept=${acceptScore}, revision=${revisionScore})`)
        return { verdict: "revisions", body: trimmed, heuristic: true }
    }
