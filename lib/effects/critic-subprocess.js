...
The three key files are embedded below. You do NOT need to Read them again. The task directory is also on --add-dir so you can read progress.md / revisions/ / any other files the report references if needed, and run QUICK bash commands (git diff, file reads, grep) to spot-check claims — but do NOT run tests, builds, linters, pre-commit, or any command that takes more than 10 seconds. Trust the worker's test output if it looks plausible; your job is to catch logical gaps and shirking, not to re-run the entire test suite.
