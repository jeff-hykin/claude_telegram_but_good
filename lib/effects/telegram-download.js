...
export async function downloadTelegramFile(effect, core) {
    ...
    if (!ok) {
        ...
        if (chatId && core.bot) {
            ...
            try {
                // Render to entities + plain (no parse_mode) like every other
                // formatted send — see lib/pure/telegram-markdown.js.
                const { text: warnText, entities } = renderMarkdownToEntities(warning)
                const opts = { format: "plain", entities }
                ...
                await core.bot.sendText(chatId, warnText, opts)
            } catch (e) {
