...
// How long the dtach log must be quiet before we consider the worker
// idle. Used for both the initial "is claude at the prompt yet" check
// and the guard before injecting revision feedback.
...
// injecting anyway (claude usually shows its prompt in ~3-5 s).
...
/**
 * Wait for the dtach log to have content AND have been quiet for at
 * least LOG_QUIET_MS. Returns true if the condition was met, false if
 * we timed out at READINESS_MAX_MS.
 */
...
async function dtachInject(sockPath, text) {
    try {
        // Send \r\n matching dtach-outbound.js's convention.
...
export async function spawnScheduledTaskWorker(effect, core) {
    ...
    ;(async () => {
        ...
        // Best-effort kill of the hanging dtach session so we don't
        // leak a zombie claude process.
        try {
            await $`dtach -p ${sockPath}`.stdinText("\x03\x03\nexit\n").timeout(3000)
        } catch (e) {
            dbg("SCHED-WORKER", "budget-exhaust kill failed:", e)
        }
