...
/**
 * True while this run is still the task's live run. Goes false once the
 * stuck-watchdog reaps it (or it completes any other way) — the watch
 * loops poll this so a reaped run's coroutine doesn't outlive it and
 * fire a bogus overflow (which sets skipNext and eats the next day's run).
 */
function runIsStillLive(core, chatId, scheduleTaskId, runIso) {
    const task = core?.specialData?.scheduledTaskByChatId?.[chatId]?.[scheduleTaskId]
    return task?.state === "running" && task?.currentRun?.runIso === runIso
}
...
export async function spawnScheduledTaskWorker(effect, core) {
    ...
    const instructions = [
        ...
        `## Telegram formatting`,
        ``,
        `\`response.md\` (and any \`reply\` tool calls you make) is sent as Telegram **legacy Markdown** (\`parse_mode: "Markdown"\`).`,
        `Supported: \`*bold*\`, \`_italic_\`, backtick-delimited code spans, triple-backtick code fences, and \`[text](url)\` links. No HTML tags — \`<b>\` and friends are NOT processed, they render literally.`,
        `Escape \`*\`, \`_\`, \`\\\`\`, \`[\`, \`\\\\\` in interpolated user content (unbalanced \`*\` or \`_\` causes Telegram to reject the message).`,
    ].join("\n")
    ...
    ;(async () => {
        ...
        while (Date.now() - started < budgetMs) {
            ...
            if (!runIsStillLive(core, chatId, scheduleTaskId, runIso)) {
                dbg("SCHED-WORKER", `run ${runIso} no longer live; stopping watcher for ${scheduleTaskId}`)
                return
            }
...
export async function injectScheduledTaskText(effect, core) {
    ...
    ;(async () => {
        ...
        while (Date.now() - started < budgetMs) {
            ...
            if (!runIsStillLive(core, chatId, scheduleTaskId, runIso)) {
                dbg("SCHED-WORKER", `run ${runIso} no longer live; stopping revision watcher for ${scheduleTaskId}`)
                return
            }
            await new Promise((r) => setTimeout(r, REPORT_POLL_MS))
        }
