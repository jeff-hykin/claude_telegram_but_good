...
const { computeNextFire, resolveResumeFire } = await versionedImport("../scheduler/index.js", import.meta)
const { setScheduleTimer, clearScheduleTimer } = await versionedImport("../scheduler/timer-registry.js", import.meta)

/**
 * effect shape: { type: "schedule_timer_set", chatId, scheduleTaskId, rule, from?, resumeAt? }
 *
 ...
 *
 * `resumeAt` is the fire time persisted before a restart; when given it
 * wins over the rule, so restarting never postpones a schedule.
 *
...
export async function scheduleTimerSet(effect, core) {
    const { chatId, scheduleTaskId, rule, from, resumeAt } = effect
    ...
    // (which resets each time we create a new RRule with a fresh dtstart).
    if (rule.count != null) {
        const task = core.specialData?.scheduledTaskByChatId?.[chatId]?.[scheduleTaskId]
        const totalRuns = task?.tracking?.totalRuns ?? 0
        if (totalRuns >= rule.count) {
            dbg("SCHED-TIMER", `count limit reached for ${scheduleTaskId} (${totalRuns}/${rule.count}); no timer set`)
            return {
                stateChanges: {
                    specialData: {
    ...
    const resume = resolveResumeFire(resumeAt, rule)
    let next = resume?.at ?? null
    if (resume) {
        dbg("SCHED-TIMER", `${scheduleTaskId} resuming at ${next.toISOString()}${resume.missed ? " (missed while down)" : ""}`)
    }
    if (!next) {
        try {
