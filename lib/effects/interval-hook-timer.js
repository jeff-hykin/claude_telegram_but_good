...
/**
 * effect shape: { type: "interval_hook_timer_set", chatId, hookId, rule, from?, resumeAt? }
 *
 ...
 * wins over the rule, so restarting never postpones a hook.
 */
export async function intervalHookTimerSet(effect, core) {
    const { chatId, hookId, rule, from, resumeAt } = effect
    ...
    if (resume) {
        dbg("IHOOK-TIMER", `${hookId} resuming at ${next.toISOString()}${resume.missed ? " (missed while down)" : ""}`)
    }
    if (!next) {
        try {
