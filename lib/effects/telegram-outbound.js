...
export async function sendFileToUser(effect, core) {
    ...
    try {
        ...
        dbg("TG-OUT", `sendFile failed (code=${e?.error_code} parse=${_isParseError(e)}): ${e?.description ?? e?.message}`)
        // Parse error in caption: retry without HTML formatting on the caption.
...
export async function editTelegramMessage(effect, core) {
    ...
    try {
        ...
        dbg("TG-OUT", `editText failed (code=${e?.error_code} parse=${_isParseError(e)}): ${e?.description ?? e?.message}`)
        if (_isParseError(e)) {
...
// shapes:
//
//   1. HTML parse errors (400 "can't parse entities") — agent body
//      contains literal `<id>...</id>`, `<name>`, etc. that confuses
...
async function _recoverFailedSend(originalError, label, core, chatId, piece, sendOptions) {
    dbg("TG-OUT", `${label} failed (code=${originalError?.error_code} parse=${_isParseError(originalError)}): ${originalError?.description ?? originalError?.message}`)
