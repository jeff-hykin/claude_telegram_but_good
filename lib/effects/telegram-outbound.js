 ...
 */
function maybePrependGeneralHeader(text, chatId, threadId, recordAs, options, core) {
    ...
    const lines = []
    if (sessionId) {
        const titlePart = title && title !== sessionId ? ` ${isHtml ? `<i>${esc(title)}</i>` : `(${title})`}` : ""
        lines.push(isHtml ? `<b>/chat_${sessionId}</b>${titlePart}` : `/chat_${sessionId}${titlePart}`)
    } else {
        lines.push(isHtml ? "<b>(no session source recorded)</b>" : "(no session source recorded)")
    }
    if (topicName) { lines.push(isHtml ? `topic: <code>${esc(topicName)}</code>` : `topic: ${topicName}`) }
    if (cwd) { lines.push(isHtml ? `cwd: <code>${esc(cwd)}</code>` : `cwd: ${cwd}`) }
    if (gitBranch) { lines.push(isHtml ? `branch: <code>${esc(gitBranch)}</code>` : `branch: ${gitBranch}`) }
    if (pid) { lines.push(`pid: ${pid}`) }
    lines.push(isHtml ? `⚠️ <i>${GENERAL_HEADER_SENTINEL} — no topic thread bound</i>` : `(${GENERAL_HEADER_SENTINEL} — no topic thread bound)`)
...
export async function sendFileToUser(effect, core) {
    ...
    let sent
    try {
        // Outbound-layer guard for file captions too.
        const finalThreadId = replyToThreadId ?? options?.message_thread_id ?? null
        const guardedCaption = caption
            ? maybePrependGeneralHeader(caption, chatId, finalThreadId, recordAs, { parse_mode: "HTML" }, core)
            : caption
        const fileOpts = {
            filename,
            caption: guardedCaption,
            format: guardedCaption ? "html" : undefined,
        }
...
/**
 * Best-effort recovery from a failed sendText. Currently handles two
 * common failure modes:
 *
 *   1. HTML parse errors (400 "can't parse entities") — agent's text
 *      contains a literal `<id>...</id>`, `<name>`, etc. that confuses
 *      Telegram's HTML parser. Retry with parse_mode/format stripped
 *      (plain text). The user gets the content, just unformatted.
 *
 *   2. Anything else — give up on the send but post a single "delivery
 *      failed" notice in plain text so the user isn't left wondering
 *      why an agent went silent. Truncated preview + the error code.
 *
 * Returns the same `{ messageId }` shape as `sendText` on success, or
 * `null` if even the recovery attempt failed (caller logs and moves on).
 */
async function _recoverFailedSend(originalError, label, core, chatId, piece, sendOptions) {
    const desc = String(originalError?.description ?? originalError?.message ?? "")
    const code = originalError?.error_code
    const isParseError = code === 400 && /can't parse entities|parse entities|Unsupported start tag/i.test(desc)
    dbg("TG-OUT", `${label} failed (code=${code} parse=${isParseError}): ${desc}`)

    if (isParseError) {
        // Strip parse_mode + format so Telegram treats the body as plain text.
        ...
        try {
            ...
            // fall through to the failure-notice path below
        }
    }

    // Non-parse error, or parse-error retry also failed. Send a single
    // plain-text failure notice so the user knows something went wrong.
    // Keep the notice tiny so it can't itself trip a chunk-size limit.
    try {
        const preview = piece.replace(/\s+/g, " ").slice(0, 120)
        const notice = `[message delivery failed: ${desc.slice(0, 100)}]\nbody preview: ${preview}${piece.length > 120 ? "…" : ""}`
        const noticeOpts = { ...sendOptions }
        delete noticeOpts.parse_mode
        delete noticeOpts.format
        const sent = await core.bot.sendText(chatId, notice, noticeOpts)
        return sent
    } catch (e) {
