 ...
 *
 * `submit` (default true) appends the Enter keystroke; `atomic` (default
 * false) writes the string in one go instead of char-by-char. Arrow keys
 * set both to false/true respectively — see lib/dtach-inject.js.
 */
export async function sendRawInputToClaude(effect, core) {
    ...
    try {
        ...
        dbg("DTACH-OUT", `injected raw input to ${sessionId} (${text.length} chars, submit=${submit})`)
    } catch (e) {
