...
const {
    ...
    isShimInstalled,
    installShim,
} = await versionedImport("../../event-generators/cli/shim-setup.js", import.meta)
...
function checkAndHeal(shimPath) {
    if (isShimInstalled(shimPath)) {
        dbg("SHIM-WATCH", "shim still intact")
        return
    }
    dbg("SHIM-WATCH", "shim missing or clobbered — reinstalling")
    const result = installShim(shimPath)
    if (result.ok) {
