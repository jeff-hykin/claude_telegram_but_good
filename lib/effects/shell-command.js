...
//
// Triggered by chat-user.js when an inbound message starts with `#`.
...
//
// `# cd <path>` is handled INLINE in chat-user.js (no fork) — this
// effect only handles command execution.
// ---------------------------------------------------------------------------
...
function formatInline(cmd, cwd, code, signal, stdout, stderr, cancelled) {
    const header = cancelled
        ? `<b>$ ${escapeHtml(cmd)}</b>  <i>[cancelled${signal ? ` ${signal}` : ""}]</i>`
        : `<b>$ ${escapeHtml(cmd)}</b>${code !== 0 ? ` <i>(exit ${code}${signal ? ` ${signal}` : ""})</i>` : ""}`
    const parts = [header, `<i>cwd: ${escapeHtml(cwd)}</i>`]
    if (stdout && stdout.length > 0) {
        parts.push(`<pre>${escapeHtml(stdout)}</pre>`)
    }
    if (stderr && stderr.length > 0) {
        parts.push(`<i>stderr:</i>\n<pre>${escapeHtml(stderr)}</pre>`)
    }
    if ((!stdout || stdout.length === 0) && (!stderr || stderr.length === 0)) {
        parts.push("<i>(no output)</i>")
    }
    return parts.join("\n")
}
...
export async function spawnShellCommand(effect, core) {
    ...
    if (core.activeShellProcs.has(key)) {
        ...
        await telegramOutbound.sendTextMessageToUser({
            ...
            replyTo,
            text: `Already running: <code>${escapeHtml(existing.cmd)}</code>. Use /cancel to abort.`,
    ...
    try {
        ...
        await telegramOutbound.sendTextMessageToUser({
            type: "send_text_to_user",
            replyTo,
            text: `Spawn failed: ${escapeHtml(msg)}`,
            options: { parse_mode: "HTML" },
        }, core)
        return
    }
    ...
    ;(async () => {
        ...
        try {
            ...
            await telegramOutbound.sendTextMessageToUser({
                type: "send_text_to_user",
                replyTo,
                text: `Output too large (${body.length} chars) and could not stage to ${escapeHtml(filePath)}: ${escapeHtml(e instanceof Error ? e.message : String(e))}`,
                options: { parse_mode: "HTML" },
            }, core)
            return
        }
