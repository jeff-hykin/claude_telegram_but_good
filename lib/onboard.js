...
import { STATE_DIR, ACCESS_FILE, PID_FILE, LOCAL_REPO, HOOK_PATH } from "./protocol.js"
...
export async function onboard() {
    ...
    try {
        ...
        const preMatcher = settings.hooks.PreToolUse.find(m => m.matcher === "*" && m.hooks?.some(h => h.command === HOOK_PATH))
        if (!preMatcher) {
        ...
        const postMatcher = settings.hooks.PostToolUse.find(m => m.matcher === "*" && m.hooks?.some(h => h.command === HOOK_PATH))
        if (!postMatcher) {
