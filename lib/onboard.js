...
// Read the hook script from the run/ directory at import time (works for local installs)
// For URL installs, onboard fetches it at runtime
let HOOK_SCRIPT
try {
    HOOK_SCRIPT = Deno.readTextFileSync(sibling(import.meta, "../run/hook"))
} catch {
    HOOK_SCRIPT = null
}
...
export async function onboard() {
    ...
    // Install the official plugin first so Claude registers it properly
    info("Installing telegram plugin via Claude CLI...")
    ...
    try {
        ...
        const out = new TextDecoder().decode(installResult.stdout).trim()
        const err = new TextDecoder().decode(installResult.stderr).trim()
        if (installResult.success) {
            ok("Plugin installed: " + out)
        } else {
            // May already be installed
            info("Plugin install output: " + (out || err))
        }
    } catch (err) {
        fail(`Plugin install failed: ${err}`)
    }

    // Find the cached plugin dir and symlink our repo over it
    const pluginCacheBase = join(HOME, ".claude", "plugins", "cache", "claude-plugins-official", "telegram")
    let pluginCacheDir = null
    try {
        // Find the versioned subdirectory (e.g. 0.2.0)
        for (const entry of Deno.readDirSync(pluginCacheBase)) {
            if (entry.isDirectory || entry.isSymlink) {
                pluginCacheDir = join(pluginCacheBase, entry.name)
            }
        }
    } catch { /* ignore */ }

    if (!pluginCacheDir) {
        fail("Could not find plugin cache directory after install")
    } else {
        // Determine plugin source
        const metaUrl = new URL(".", import.meta.url)
        let pluginSrcDir
        if (metaUrl.protocol === "file:") {
            pluginSrcDir = sibling(import.meta, "..")
        } else {
            pluginSrcDir = join(HOME, ".local", "share", "cbg", "plugin")
            info("Downloading plugin files...")
            try {
                Deno.mkdirSync(pluginSrcDir, { recursive: true })
                const repoBase = import.meta.url.replace(/\/lib\/onboard\.js$/, "").replace(/\/mod\.js$/, "")
                const files = [
                    ".mcp.json", ".claude-plugin/plugin.json", ".claude-plugin/marketplace.json",
                    "imports.js", "shim.js", "standalone-server.js", "mod.js",
                    "lib/protocol.js", "lib/config.js", "lib/access.js", "lib/commands.js",
                    "lib/telegram-api.js", "lib/hooks.js", "lib/daemon.js", "lib/dtach.js",
                    "lib/onboard.js", "lib/shim.js", "lib/names.js",
                    "vendor/mcp-server.js", "vendor/mcp-stdio.js", "vendor/mcp-types.js",
                    "commands/_shared.js", "commands/approve_user.js", "commands/cancel.js",
                    "commands/cron.js", "commands/help.js", "commands/hooks.js",
                    "commands/kill.js", "commands/list.js", "commands/new_command.js",
                    "commands/pause.js", "commands/ping.js", "commands/spawn.js",
                    "commands/start.js", "commands/status.js", "commands/title.js",
                    "commands/version.js", "run/hook", "run/start", "run/server",
                    "skills/access/SKILL.md", "skills/configure/SKILL.md", "skills/logs/SKILL.md",
                ]
                for (const f of files) {
                    const dir = join(pluginSrcDir, f, "..")
                    Deno.mkdirSync(dir, { recursive: true })
                    try {
                        const content = await fetch(`${repoBase}/${f}`).then(r => r.text())
                        Deno.writeTextFileSync(join(pluginSrcDir, f), content)
                    } catch { /* skip optional files */ }
                }
                for (const f of ["run/hook", "run/start", "run/server"]) {
                    try { Deno.chmodSync(join(pluginSrcDir, f), 0o755) } catch { /* ignore */ }
                }
                ok("Plugin files downloaded.")
            } catch (err) {
                fail(`Download failed: ${err}`)
            }
        }

        // Symlink our repo over the cache dir
        try {
            Deno.removeSync(pluginCacheDir, { recursive: true })
            Deno.symlinkSync(pluginSrcDir, pluginCacheDir)
            ok("Plugin cache symlinked to local repo.")
        } catch (err) {
            fail(`Symlink failed: ${err}`)
        }
    }

    // Also symlink the marketplace source dir (external_plugins/telegram)
    const marketplacePluginDir = join(
        HOME, ".claude", "plugins", "marketplaces",
        "claude-plugins-official", "external_plugins", "telegram",
    )
    try {
        try { Deno.removeSync(marketplacePluginDir, { recursive: true }) } catch { /* ignore */ }
        Deno.mkdirSync(join(marketplacePluginDir, ".."), { recursive: true })
        Deno.symlinkSync(pluginSrcDir, marketplacePluginDir)
        ok("Marketplace source symlinked.")
    } catch (err) {
        info(`Marketplace symlink skipped: ${err}`)
    }
    ...
    try {
        ...
        // Write hook script to a known local path
        const hookPath = join(HOME, ".claude", "channels", "telegram", "bin", "hook")
        Deno.mkdirSync(join(hookPath, ".."), { recursive: true })
        let hookSrc = HOOK_SCRIPT
        if (!hookSrc) {
            // URL install — fetch the hook script
            const hookUrl = sibling(import.meta, "../run/hook")
            hookSrc = await fetch(hookUrl).then(r => r.text())
        }
        Deno.writeTextFileSync(hookPath, hookSrc)
        ...
        const preMatcher = settings.hooks.PreToolUse.find(m => m.matcher === "*" && m.hooks?.some(h => h.command?.includes("run/hook")))
        if (!preMatcher) {
            settings.hooks.PreToolUse.push({ matcher: "*", hooks: [hookEntry] })
        }
        // PostToolUse — match all tools
        if (!settings.hooks.PostToolUse) {
            settings.hooks.PostToolUse = []
        }
        const postMatcher = settings.hooks.PostToolUse.find(m => m.matcher === "*" && m.hooks?.some(h => h.command?.includes("run/hook")))
        if (!postMatcher) {
            settings.hooks.PostToolUse.push({ matcher: "*", hooks: [hookEntry] })
        }
    ...
    const permFile = join(HOME, ".claude", "channels", "telegram", "permission_args")
    Deno.mkdirSync(join(permFile, ".."), { recursive: true })
    let permArgs = ""
    if (permChoice === "all") {
    ...
    // --- Step 6: Telegram pairing ---
    header("6", "Telegram Pairing")
    ...
    if (existingAllowFrom.length > 0) {
        ...
        if (!repair) {
            // Skip to shim
            await doShimAndVerify(pluginCacheDir)
            return
        }
    }
