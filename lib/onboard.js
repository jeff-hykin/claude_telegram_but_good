...
import { startService, stopService, isDaemonRunning } from "./daemon.js"
import { HOME, STATE_DIR, ACCESS_FILE, PID_FILE, LOCAL_REPO, HOOK_PATH, CONFIG_FILE, dbg } from "./protocol.js"
import { buildPatchedMcpJson } from "./plugin-patch.js"
const c = colors
...
/**
 * Remove cbg hooks and plugin from settings.json.
 */
export function removeFromSettingsJson() {
    ...
    try {
        ...
        dbg("UNINSTALL", "could not read settings.json:", e)
        return
    }

    // Remove hooks that point to our hook path
    for (const event of ["PreToolUse", "PostToolUse"]) {
        if (Array.isArray(settings.hooks?.[event])) {
            settings.hooks[event] = settings.hooks[event].filter(
                m => !m.hooks?.some(h => h.command === HOOK_PATH || h.command?.includes("cbg"))
            )
        }
    }
    ...
    const ep = settings.enabledPlugins
    if (Array.isArray(ep)) {
        settings.enabledPlugins = ep.filter(p => p !== pluginId)
    } else if (ep && typeof ep === "object") {
        delete ep[pluginId]
    }
