...
export async function onboard() {
    ...
    try {
        ...
        // Write bash wrapper that finds deno and runs the JS hook
        ...
        Deno.writeTextFileSync(hookPath, generateHookScript(hookJsPath))
        ...
        const preMatcher = settings.hooks.PreToolUse.find(m => m.matcher === "*" && m.hooks?.some(h => h.command?.includes("bin/hook")))
        if (!preMatcher) {
        ...
        const postMatcher = settings.hooks.PostToolUse.find(m => m.matcher === "*" && m.hooks?.some(h => h.command?.includes("bin/hook")))
        if (!postMatcher) {
