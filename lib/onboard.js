...
export async function onboard() {
    ...
    const passcodeFile = join(STATE_DIR, "pending_passcode.json")
    Deno.mkdirSync(STATE_DIR, { recursive: true })
    Deno.writeTextFileSync(passcodeFile, JSON.stringify({
        code: passcode,
    ...
    if (!serverReady) {
        ...
        try { Deno.removeSync(passcodeFile) } catch { /* ignore */ }
        Deno.exit(1)
    }
    ...
    console.log(c.bold.white("    Open your bot in Telegram and send this message:"))
    console.log()
    console.log(c.bold.cyan(`      /passcode ${passcode}`))
    console.log()
    info("Waiting for pairing...")
    info(c.dim("(passcode expires in 10 minutes)"))
    console.log()
    ...
    // Clean up passcode file in case it wasn't consumed
    try { Deno.removeSync(passcodeFile) } catch { /* ignore */ }
    ...
    if (paired) {
        ...
        warn("Pairing timed out. You can pair later by running `cbg onboard` again.")
    }
