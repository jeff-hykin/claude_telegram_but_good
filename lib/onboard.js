 ...
 */
/**
 * Install the telegram plugin via Claude CLI and symlink the local repo over it.
 * Clones/updates the repo to ~/.local/share/cbg/plugin, then symlinks it into
 * the Claude plugin cache and external_plugins directories.
...
export function installAndSymlinkPlugin() {
    ...
    const localRepo = join(HOME, ".local", "share", "cbg", "plugin")
    ...
    try {
        ...
        try { Deno.removeSync(localRepo, { recursive: true }) } catch { /* ignore */ }
    ...
    // Symlink local repo over both cache and external_plugins
    for (const target of [cacheDir, extDir]) {
        try { new Deno.Command("chmod", { args: ["-h", "+w", target], stdout: "null", stderr: "null" }).outputSync() } catch { /* ignore */ }
        try { Deno.removeSync(target, { recursive: true }) } catch { /* ignore */ }
        Deno.mkdirSync(join(target, ".."), { recursive: true })
        Deno.symlinkSync(localRepo, target)
        try { new Deno.Command("chmod", { args: ["-h", "-w", target], stdout: "null", stderr: "null" }).outputSync() } catch { /* ignore */ }
    }
