...
// structured telegram message history (JSONL, one message per line)
...
export function dbg(label, ...args) {
    ...
    try {
        ...
        Deno.stderr.writeSync(encoder.encode(`[TG-DBG ${ts}] LOG: write to ${LOG_FILE} failed: ${e}\n`))
    }
}

/**
 * Append a structured Telegram message record to MESSAGES_FILE (JSONL).
 * Used by /doctor and any future tooling that wants a global history of
 * what flowed through the bot without scraping main.log.
 *
 * direction: "in" (user -> bot) or "out" (bot -> user)
 */
export function logMessage(record) {
    try {
        const entry = { ts: new Date().toISOString(), ...record }
        Deno.writeTextFileSync(MESSAGES_FILE, JSON.stringify(entry) + "\n", { append: true })
    } catch (e) {
        dbg("MSGLOG", "failed to append message:", e)
    }
}
