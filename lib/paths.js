...
export function buildPaths({ cbgDir, claudeDir }) {
    ...
    return {
        ...
        // Transcript of a non-TUI agent-runner session. Plays the role
        // the dtach log plays for Claude: what /peek renders.
        localAgentLogFile(sessionId) {
            return join(stateDir, `local-agent-${sessionId}.log`)
        },
        longTaskDir(taskId) {
