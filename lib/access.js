...
export function readAccessFile() {
    try {
        ...
        return {
            ...
            botCenterGroups: (parsed.botCenterGroups ?? []).map(String),
            groupChats: (parsed.groupChats ?? []).map(String),
...
/**
 * Which of the two group buckets a chat belongs to.
 *
 * - "botCenter" — full bot behavior: topics map to sessions, plain text
 *   reaches Claude, spinners and status messages are posted.
 * - "groupChat" — the default for every group CBG has not been told
 *   about. The bot stays completely silent unless it is addressed.
 *
 * Membership in BotCenter is explicit (`botCenterGroups`, plus the
 * legacy single `commandCenterChatId`); everything else falls through
 * to GroupChats, so a group that nobody configured can never wake the
 * bot by accident.
 */
export function classifyGroup(chatId, access) {
    const id = String(chatId)
    if (id === String(access?.commandCenterChatId ?? "")) {
        return "botCenter"
    }
    if ((access?.botCenterGroups ?? []).includes(id)) {
        return "botCenter"
    }
    return "groupChat"
}
...
/**
 * True when a message addresses the bot: an @username mention, a
 * text_mention entity, a reply to one of the bot's own messages, or a
 * configured `mentionPatterns` regex.
 *
 * The bot's username comes from Grammy's `ctx.me` (populated by
 * `bot.init()`), with `access.botUsername` as an override for adapters
 * that don't carry a Grammy context. When neither is known we fail
 * CLOSED — silence is the safe answer in an unconfigured group.
 */
export function isBotMentioned(event, access) {
    const ctx = event?._ctx
    const botUsername = ctx?.me?.username ?? access?.botUsername ?? null
    if (!ctx) {
        return false
    }
    return isMentioned(ctx, botUsername, access?.mentionPatterns)
}
...
export function gate(ctx, botUsername, staticAccess = null) {
    ...
    if (chatType === "group" || chatType === "supergroup") {
        ...
        if (classifyGroup(groupId, access) === "botCenter") {
        ...
        if (!isMentioned(ctx, botUsername, access.mentionPatterns)) {
            return { action: "drop" }
        }
        return { action: "deliver", access }
    }
