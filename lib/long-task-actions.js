...
export function buildCancelAction(core, chatId, taskId) {
    ...
    if (task.state === "cancelled") {
        return { ok: false, reason: `Task ${taskId} is already cancelled. Use /task_resume_${taskId} to resume.` }
    }
    if (task.state === "certified" || task.state === "escalated") {
        return { ok: false, reason: `Task ${taskId} is in terminal state ${task.state} and cannot be cancelled.` }
    }
    ...
    const effects = [
        {
            ...
            text:
                ...
                `Use /task_resume_${taskId} to resume.`,
            options: {},
        },
        {
            type: "cold_append",
