...
export function buildCancelAction(core, chatId, taskId) {
    ...
    if (!task) {
        return { ok: false, reason: `Task \`${esc(taskId)}\` not found.` }
    }
    if (task.state === "cancelled") {
        return { ok: false, reason: `Task \`${esc(taskId)}\` is already cancelled. Use \`/task_resume_${esc(taskId)}\` to resume.` }
    }
    if (task.state === "certified" || task.state === "escalated") {
        return { ok: false, reason: `Task \`${esc(taskId)}\` is in terminal state \`${esc(task.state)}\` and cannot be cancelled.` }
    }
    ...
    const effects = [
        {
            ...
            chatId,
            text:
                `Task \`${esc(taskId)}\` cancelled. ` +
                `Use \`/task_resume_${esc(taskId)}\` to resume.`,
