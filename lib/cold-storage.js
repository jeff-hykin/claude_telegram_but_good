 ...
 *
 * The streams under $STATE_DIR/cold-storage/:
 ...
 *   - scheduled-tasks.jsonl — scheduled-task fires, runs, and outcomes
 *   - interval-hooks.jsonl  — interval-hook fires, runs, and outcomes
 *
 * A stream name missing from VALID_STREAMS makes every append for it
 * throw, silently losing the whole history — which is what happened to
 * the two scheduler streams and is why their stats read zero.
 */
...
const [
    ...
    versionedImport("./scrub.js", import.meta),
])

// Re-exported under the legacy name used by tests/cold-storage-test.js.
export const COLD_DIR = paths.COLD_STORAGE_DIR

const VALID_STREAMS = new Set(["messages", "long-tasks", "hooks", "scheduled-tasks", "interval-hooks"])
...
export function appendColdEntry(stream, entry) {
    ...
    const scrubbed = scrubObject(stamped)
    try {
        Deno.writeTextFileSync(path, JSON.stringify(scrubbed) + "\n", { append: true })
    } catch (e) {
