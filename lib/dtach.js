 ...
 */
export function createSession(title, extraClaudeArgs = [], { detached = false } = {}) {
    ...
    if (detached) {
        // dtach -n: create socket but don't attach (background mode)
        const child = new Deno.Command("bash", {
            args: ["-c", `dtach -n "$1" -z bash -c 'claude ${claudeArgs.map(a => `'"'"'${a}'"'"'`).join(" ")} 2>&1 | tee "$2"'`, "_", dtachSock, logFile],
            env,
            stdin: "null",
            stdout: "null",
            stderr: "null",
